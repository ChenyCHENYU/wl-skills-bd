...
const FORBIDDEN_DATABASE_IS_PREFIX = /^is_/i;
const FORBIDDEN_JAVA_IS_PREFIX = /^is[A-Z0-9_]/;

function forbiddenDatabaseField(value) {
  return typeof value === "string" && FORBIDDEN_DATABASE_IS_PREFIX.test(value);
}

function forbiddenJavaField(value) {
  return typeof value === "string" && FORBIDDEN_JAVA_IS_PREFIX.test(value);
}

function databaseFlagSuggestion(value) {
  const raw = String(value || "");
  const upper = raw === raw.toUpperCase();
  const suffix = raw.replace(FORBIDDEN_DATABASE_IS_PREFIX, "");
  const suggested = `${suffix || "status"}_flag`;
  return upper ? suggested.toUpperCase() : suggested.toLowerCase();
}

function javaFlagSuggestion(value) {
  const raw = String(value || "");
  const suffix = raw.replace(/^is/, "");
  if (!suffix) return "statusFlag";
  return `${suffix.charAt(0).toLowerCase()}${suffix.slice(1)}Flag`;
}
...
function validateGovernance(profile) {
  ...
  if (forbiddenDatabaseField(softDelete.column)) {
    errors.push(`softDelete.column 禁止使用 is_ 前缀；Java Bean get/set 与序列化映射存在歧义，请改为 ${databaseFlagSuggestion(softDelete.column)}`);
  }
  if (softDelete.javaField !== "deleteFlag") {
    errors.push("softDelete.javaField 当前必须为 deleteFlag；禁止 isXxx 字段，避免 Java Bean get/set 与序列化映射歧义");
  }
...
function physicalIdentifier(database, value) {
  return database === "mysql" ? String(value).toLowerCase() : String(value).toUpperCase();
}
