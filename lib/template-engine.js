...
function render(template, context) {
  // Mustache standalone-line semantics: control tags that occupy a whole line
  // must not leak their indentation/newline into generated source.  Without
  // this rule, every optional import/annotation creates blank-line pollution.
  const standaloneTrimmed = template.replace(
    /^[ \t]*({{\s*[#^/][^{}\r\n]+}})[ \t]*\r?\n/gm,
    "$1",
  );
  return renderNodes(parse(standaloneTrimmed), [context]);
}
