...
function receiptId(entry) {
  return crypto.createHash("sha256").update(JSON.stringify({
    ...
    column: String(entry.column || "").toLowerCase(),
    ddlPlanHash: entry.ddlPlanHash,
...
function validateLedgerEntry(entry, now = Date.now()) {
  ...
  if (!/^[a-f0-9]{64}$/.test(String(entry.migrationHash || ""))) errors.push("migrationHash 必须是迁移文件内容的 64 位 sha256");
...
function buildLedgerPlan(projectRoot, entry) {
  ...
  const normalized = {
    ...
    ddlPlanHash: String(entry.ddlPlanHash || entry.planHash || "").trim(),
    migrationHash: String(entry.migrationHash || "").trim(),
  ...
  const identity = receiptId(normalized);
  const existing = ledger.executed.find((item) => item && item.receiptId === identity);
  const record = existing || { ...normalized, receiptId: identity, recordedAt: normalized.executedAt };
  const idempotent = Boolean(existing);
  const next = { executed: idempotent ? ledger.executed : [...ledger.executed, record] };
  const plan = fileTransaction.buildFilePlan(projectRoot, LEDGER_FILE, `${JSON.stringify(next, null, 2)}\n`, {
    kind: "ddl-ledger-append",
    metadata: { entry: record, total: next.executed.length, idempotent },
  });
  return { ...plan, entry: record, total: next.executed.length, idempotent, receiptId: identity };
}

function publicLedgerPlan(plan) {
  if (!plan.ok) return plan;
  return {
    ...fileTransaction.publicFilePlan(plan),
    entry: plan.entry,
    total: plan.total,
...
function applyLedgerPlan(plan, options = {}) {
  const result = fileTransaction.applyFilePlan(plan, options);
  return {
    ...result,
    total: plan.total,
    idempotent: plan.idempotent,
    receiptId: plan.receiptId,
    file: normalizeRel(LEDGER_FILE),
  };
}
...
function approvedFromLedger(ledger) {
  ...
  for (const e of ledger.executed) {
    ...
    if (validateLedgerEntry(e).length > 0) continue;
...
/** 快照导出辅助（v0.26）：给 DBA/只读账号的导出 SQL 与 JSON 格式模板，降低 drift 对账摩擦。 */
function snapshotTemplate(database) {
  const db = database === "mysql" ? "mysql" : "oracle";
  const sql = db === "mysql"
    ? [
      "-- MySQL：导出指定库的全表结构快照（只读，替代手工 DESC）",
      "SELECT t.TABLE_NAME AS `table`,",
      "       c.COLUMN_NAME AS name, c.ORDINAL_POSITION AS ordinal, c.COLUMN_TYPE AS dbType,",
      "       (c.IS_NULLABLE = 'YES') AS nullable, c.COLUMN_DEFAULT AS defaultValue, c.COLUMN_COMMENT AS comment",
    ...
    : [
      "-- Oracle：导出当前用户的全表结构快照（只读）",
      "SELECT t.TABLE_NAME AS \"table\",",
      "       c.COLUMN_NAME AS name, c.COLUMN_ID AS ordinal,",
      "       c.DATA_TYPE || CASE WHEN c.DATA_PRECISION IS NOT NULL THEN '(' || c.DATA_PRECISION || CASE WHEN c.DATA_SCALE > 0 THEN ',' || c.DATA_SCALE ELSE '' END || ')' ELSE '' END AS dbType,",
      "       (c.NULLABLE = 'Y') AS nullable, c.DATA_DEFAULT AS defaultValue,",
      "       (SELECT cc.COMMENTS FROM USER_COL_COMMENTS cc WHERE cc.TABLE_NAME = c.TABLE_NAME AND cc.COLUMN_NAME = c.COLUMN_NAME) AS comment",
      "FROM USER_TABLES t",
  ...
  return [
    "# 数据库结构快照导出指南",
    "",
    "目的：为 `wl-skills-bd db drift --snapshot <file>` 提供与文档镜像/契约/Flyway 精确对账的快照。",
    "",
    "## 第 1 步：执行导出 SQL（" + db + "，只读）",
    "",
    "```sql",
    sql,
    "```",
    "",
    "## 第 2 步：整理为以下 JSON 格式（推荐携带全属性；columns 仅名称数组也可用但会降低对账精度）",
    "",
    "```json",
    "[",
    "  {",
    ...
    '    "columns": [',
    '      { "name": "ID", "ordinal": 1, "dbType": "NUMBER(19)", "nullable": false, "defaultValue": null, "comment": "主键ID" },',
    '      { "name": "FEATURE_NAME", "ordinal": 2, "dbType": "VARCHAR2(100)", "nullable": false, "defaultValue": null, "comment": "特征名称" }',
    "    ]",
    "  }",
    "]",
    "```",
    ...
    "```",
    "",
    "> 安全边界：bd 不连接数据库；快照由 DBA/只读账号导出后离线对账。",
  ].join("\n");
}
