...
function toSarif(result) {
  ...
  return `${JSON.stringify({
    ...
    runs: [{
      ...
      invocations: [{
        executionSuccessful: result.stats.error === 0 && (!result.coverage || result.coverage.status === "complete"),
        properties: result.coverage ? { status: result.status, coverage: result.coverage } : undefined,
      }],
    }],
  }, null, 2)}\n`;
}
...
function toMarkdown(result) {
  const lines = [
    ...
    ...(result.coverage ? [`- 覆盖：${result.coverage.status}（${result.coverage.mode}）；skipped=${result.coverage.skippedRules.map((item) => item.rule).join(",") || "none"}`] : []),
...
function toText(result) {
  ...
  if (result.coverage) lines.push(`status=${result.status || result.coverage.status} mode=${result.coverage.mode} skipped=${result.coverage.skippedRules.map((item) => item.rule).join(",") || "none"}`);
