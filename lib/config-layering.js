...
function parseYamlKeyValue(content) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (kvMatch) {
      ...
      if (separator === ":" && (value === "" || value === "|" || value === ">")) {
...
function isPlaintextSecret(value) {
  ...
  if (/^(?:true|false|null|~|[-+]?\d+(?:\.\d+)?)$/i.test(value.trim())) return false;
...
function detectBootstrapLayer(root) {
  ...
  for (const file of candidates) {
    ...
    try {
      ...
      const datasourceDataIds = kvs
        .filter((kv) => kv.key === "dataId" && /^datasource-/i.test(kv.value))
        .map((kv) => kv.value);
      return {
        layer: "L1",
        ...
        profileDeclared: Boolean(profile),
