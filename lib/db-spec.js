  ...
  for (const entry of entries) {
    ...
    for (const rawTable of parsed.tables) {
      ...
      for (const field of table.fields) {
        if (forbiddenDatabaseField(field.name)) {
          result.errors.push({
            code: "FORBIDDEN_IS_PREFIX_FIELD",
            message: `${table.name}.${field.name} 使用禁用的 is_ 前缀；Java Bean get/set 与序列化映射存在歧义，禁止据此建表，请先与业务确认改为 ${databaseFlagSuggestion(field.name)}`,
          });
        }
      }
...
function delegatedSource(projectRoot) {
  const readme = path.join(projectRoot, DB_SPEC_DIR, "README.md");
  if (!fs.existsSync(readme)) return null;
  let content;
  try { content = fs.readFileSync(readme, "utf8"); } catch { return null; }
  const sources = [];
  for (const match of content.matchAll(/\]\(([^)]+\.json(?:#[^)]*)?)\)/gi)) {
    const link = match[1].split("#")[0];
    if (/^(?:https?:|[A-Za-z][A-Za-z0-9+.-]*:)/i.test(link)) continue;
    const candidate = path.resolve(path.dirname(readme), decodeURIComponent(link));
    ...
    try {
      ...
      sources.push(normalizePath(relative));
    } catch { /* malformed sources remain ordinary missing-spec diagnostics */ }
  }
  return sources.length > 0 ? { readme: normalizePath(path.relative(projectRoot, readme)), sources: [...new Set(sources)].sort() } : null;
}
...
function descriptorFromContract(contract, source = "wl-contract.json", profile) {
  ...
  const idType = databaseIdType(database, profile);
  const actorType = databaseActorType(database, profile);
  ...
  const platformFields = [
    { name: column("ID"), dbType: idType, nullable: false, comment: "主键ID" },
    { name: column("COMPANY_ID"), dbType: idType, nullable: false, comment: "公司/租户ID" },
    {
    ...
    { name: column("CREATE_USER_NO"), dbType: actorType, nullable: auditColumns.createUserNullable, comment: "创建人工号" },
    { name: column("UPDATE_USER_NO"), dbType: actorType, nullable: auditColumns.updateUserNullable, comment: "更新人工号" },
    { name: column("CREATE_DATE_TIME"), dbType: auditType, nullable: auditColumns.createTimeNullable, comment: "创建时间" },
    { name: column("UPDATE_DATE_TIME"), dbType: auditType, nullable: auditColumns.updateTimeNullable, comment: "更新时间" },
...
function compare(projectRoot, contractTables, options = {}) {
  ...
  const issues = [];
  if (!spec.ok) {
    const namingErrors = spec.errors.filter((error) => error.code === "FORBIDDEN_IS_PREFIX_FIELD");
    for (const error of namingErrors) {
      issues.push({ severity: strictMissing ? "error" : "warn", file: DB_SPEC_DIR, message: error.message });
    }
    if (namingErrors.length > 0) {
      return { ok: !strictMissing, issues, spec, governance, fingerprint: null };
    }
    const delegated = delegatedSource(projectRoot);
    if (delegated) {
      issues.push({
        severity: "warn",
        file: delegated.readme,
        message: `已登记委托数据库事实源 ${delegated.sources.join(", ")}；本次未执行 docs/db-spec 原生字段镜像比对`,
      });
      return { ok: true, issues, spec, governance, delegated, fingerprint: null };
    }
...
/**
 * 三方字段对账（v0.26）：文档镜像(docs/db-spec) ↔ 契约 ↔ 可选线上快照。
 * 与 compare() 的"错了才报"不同，这里输出每个字段的正向对账行，供 apply 前人工复核。
 */
function reconcileContract(projectRootInput, contract, options = {}) {
  ...
  const spec = loadDbSpec(projectRoot);
  const governance = loadGovernance(projectRoot);
  const extensions = governance.extensionFields.get(descriptor.key) || new Map();
  const specTable = spec.ok ? spec.tables.get(descriptor.key) || null : null;
  const snapshotTables = options.snapshotTables || null;
  const snapshotTable = snapshotTables ? snapshotTables.get(descriptor.key) || null : null;
  ...
  const differs = (left, right, property) => {
    if (property === "dbType") {
      return left.dbType !== undefined && normalizeType(left.dbType) !== normalizeType(right.dbType);
    }
    if (property === "defaultValue") {
      return left.defaultValue !== undefined && normalizeDefault(left.defaultValue) !== normalizeDefault(right.defaultValue);
    }
    if (property === "nullable") {
      return left.nullable !== undefined && left.nullable !== right.nullable;
    }
    if (property === "comment") {
      return Boolean(left.comment) && left.comment !== right.comment;
    }
    return false;
  };

  const specByKey = new Map((specTable ? specTable.fields : []).map((field) => [field.key, field]));
  const contractBusiness = descriptor.fields.filter((field) => !PLATFORM_COLUMNS.has(field.key));
  const contractByKey = new Map(contractBusiness.map((field) => [field.key, field]));
  const snapshotByKey = new Map((snapshotTable ? snapshotTable.fields : []).map((field) => [field.key, field]));
  const platformByKey = new Map((descriptor.platformFields || []).map((field) => [field.key, field]));

  const rowKeys = [];
  const pushKey = (key) => { if (!rowKeys.includes(key)) rowKeys.push(key); };
  if (specTable) for (const field of specTable.fields) pushKey(field.key);
  for (const field of contractBusiness) pushKey(field.key);
  if (snapshotTable) for (const field of snapshotTable.fields) pushKey(field.key);
  for (const key of PLATFORM_COLUMNS) {
    if (specByKey.has(key) || contractByKey.has(key) || platformByKey.has(key) || snapshotByKey.has(key)) pushKey(key);
  }

  const rows = rowKeys.map((key) => {
    const doc = specByKey.get(key) || null;
    const actual = contractByKey.get(key) || platformByKey.get(key) || null;
    const snap = snapshotByKey.get(key) || null;
    const origin = !doc && !actual && snap ? "snapshot-only"
      : !doc && extensions.has(key) ? "extension"
        : !doc && PLATFORM_COLUMNS.has(key) ? "platform"
          : doc && !actual ? "doc-only"
            : !doc && actual ? "contract-only"
              : "baseline";
    const issues = [];
    if (doc && actual) {
      for (const property of ["dbType", "nullable", "defaultValue", "comment"]) {
        if (differs(doc, actual, property)) {
          const waiver = waiverFor(governance, { key: descriptor.key }, { key }, property);
          issues.push({
            property,
            severity: waiver ? "warn" : "error",
            message: `${doc.name}.${property} 文档=${JSON.stringify(doc[property])} 契约=${JSON.stringify(actual[property])}${waiver ? `（已按 ${waiver.approvalRef} 例外）` : ""}`,
          });
        }
      }
    }
    if (doc && snap && differs(doc, snap, "dbType")) {
      issues.push({ property: "snapshot.dbType", severity: "error", message: `${doc.name} 线上快照类型=${JSON.stringify(snap.dbType)} 与文档=${JSON.stringify(doc.dbType)} 不一致` });
    }
    if (!doc && !snap && actual && extensions.has(key)) {
      const entry = extensions.get(key);
      issues.push({ property: "approval", severity: "warn", message: `扩展字段 ${actual.name} 依据 ${entry.sourceRef}（${entry.approvalRef}，${entry.approvedBy}）` });
    }
    return {
      field: (doc && doc.name) || (actual && actual.name) || (snap && snap.name) || key,
      origin,
      doc: doc ? propOf(doc) : null,
      contract: actual ? propOf(actual) : null,
      snapshot: snap ? propOf(snap) : null,
      issues,
    };
  });

  const mismatched = rows.filter((row) => row.issues.some((issue) => issue.severity === "error")).length;
  const warned = rows.filter((row) => row.issues.length > 0 && !row.issues.some((issue) => issue.severity === "error")).length;
  return {
    ok: mismatched === 0,
    table: descriptor.name || contract.entity?.table,
    database: contract.database,
    specConfigured: spec.ok,
    snapshotProvided: Boolean(snapshotTables),
    summary: {
      fields: rows.length,
      baseline: rows.filter((row) => row.origin === "baseline").length,
      extension: rows.filter((row) => row.origin === "extension").length,
      platform: rows.filter((row) => row.origin === "platform").length,
      docOnly: rows.filter((row) => row.origin === "doc-only").length,
      contractOnly: rows.filter((row) => row.origin === "contract-only").length,
      snapshotOnly: rows.filter((row) => row.origin === "snapshot-only").length,
      mismatched,
      warned,
    },
    rows,
