...
const { collectControllerEndpoints } = require("./be-rules");
...
function loadCatalogConfig(projectRootInput, configRel = DEFAULT_CONFIG_REL) {
  ...
  for (const [moduleId, value] of Object.entries(raw.modules || {})) {
    ...
    const moduleRoot = typeof value.root === "string" && value.root.trim() ? normalizeRel(value.root) : null;
    ...
    if (moduleRoot) {
      try { resolveWithin(projectRoot, moduleRoot); } catch (cause) { errors.push(error(`${location}.root`, cause.message)); }
    }
...
function moduleSourceEvidence(projectRoot, moduleConfig) {
  const contractRootFiles = listFiles(projectRoot, moduleConfig.contractRoots, (file) => file.toLowerCase().endsWith(".json"));
  const contractFiles = contractRootFiles.filter((rel) => {
  ...
  const files = uniqueSorted([...contractRootFiles, ...sourceFiles]);
  const evidence = files.map((rel) => {
    const file = resolveWithin(projectRoot, rel);
    const kind = contractFiles.includes(rel) ? "contract" : contractRootFiles.includes(rel) ? "contract-evidence" : "source";
    return { rel, hash: hashFile(file), bytes: fs.statSync(file).size, kind };
  });
  return { contractFiles, sourceFiles, evidence, sourceHash: hashJson(evidence) };
}

function serviceNames(contract) {
  if (!contract.rootPackage || !contract.module || !contract.entity || !contract.entity.name) {
    return { service: null, controller: null, mapper: null };
  }
...
function sourceEndpoints(projectRoot, sourceFiles) {
  const contents = new Map();
  for (const rel of sourceFiles) {
    if (!rel.endsWith(".java")) continue;
    contents.set(rel, fs.readFileSync(resolveWithin(projectRoot, rel), "utf8"));
  }
  return collectControllerEndpoints(contents);
}

function endpointContractId(endpoint, resources) {
  const controller = path.basename(endpoint.file, ".java");
  const byController = resources.find((resource) => `${resource.entity}Controller` === controller);
  if (byController) return byController.contractId;
  const normalizedPath = String(endpoint.path || "").toLowerCase();
  const byPath = resources.find((resource) => resource.apiRequestPath
    && normalizedPath.split("/").includes(resource.apiRequestPath.toLowerCase()));
  return byPath ? byPath.contractId : null;
}
...
function buildModuleCatalog(projectRoot, config, moduleId) {
  ...
  for (const contractFile of sources.contractFiles) {
    const loaded = loadContractCompat(resolveWithin(projectRoot, contractFile), { projectRoot });
    if (!loaded.ok) {
      contractErrors.push(...loaded.errors.map((item) => ({ ...item, file: contractFile })));
      continue;
    }
    contractWarnings.push(...loaded.warnings.map((item) => ({ ...item, file: contractFile })));
    const descriptor = loaded.descriptor;
    const contract = loaded.raw;
    if (descriptor.module !== moduleId) {
      contractErrors.push({ file: contractFile, path: "$.module", message: `契约模块 ${descriptor.module} 与 Catalog 模块 ${moduleId} 不一致` });
      continue;
    }
    const manifest = loaded.codegenSafe
      ? collaboration.buildManifest(loaded.strict.contract, loaded.strict.profile, loaded.strict.deliveryProfile)
      : null;
    const names = serviceNames(contract);
    const operations = Object.entries(manifest ? manifest.operations : {}).map(([name, operation]) => ({
      name,
    ...
    resources.push({
      ...
      contractKind: descriptor.contractKind,
      codegenSafe: descriptor.codegenSafe,
      contractFile,
      entity: descriptor.entity.name,
      description: descriptor.entity.description,
      rootPackage: contract.rootPackage || null,
      ownership: descriptor.ownership,
      fieldCount: descriptor.fields.length,
      errorCodes: (contract.errors || []).map((item) => item.code),
      logicalIds: (contract.logicalIds || []).map((item) => item.id),
      integrationIds: (contract.integrations || []).map((item) => item.id),
      apiMode: descriptor.apiMode,
      apiRequestPath: contract.api && contract.api.requestPath || null,
      names,
      operations,
      database: {
        engine: descriptor.database || "unassigned",
        cluster: descriptor.dbCluster || "unassigned",
        table: descriptor.entity.table,
        identity: `${descriptor.dbCluster || "unassigned"}|${descriptor.database || "unassigned"}|${descriptor.entity.table}`,
        migrationLocation: contract.output && contract.output.migration ? normalizeRel(contract.output.migration) : null,
        migrationVersion: descriptor.migrations[0] || null,
        migrationChain: descriptor.migrations,
        phase: contract.alter ? contract.alter.phase : descriptor.contractKind === "crud" ? "create" : "observed",
      },
      ...
      completion: manifest ? manifest.completion.contractStatus : "observed",
    });
  }
  resources.sort((left, right) => left.contractId.localeCompare(right.contractId));
  const services = resources
    .filter((resource) => Object.values(resource.names).some(Boolean))
    .map((resource) => ({ contractId: resource.contractId, ...resource.names }));
  const observedEndpoints = sourceEndpoints(projectRoot, sources.sourceFiles);
  const observedApis = observedEndpoints.map((endpoint) => ({
    contractId: endpointContractId(endpoint, resources),
    name: endpoint.handler,
    method: endpoint.method,
    internalPath: endpoint.path,
    externalPath: endpoint.path,
    permission: null,
    requestModel: null,
    responseModel: null,
  ...
  const declaredApis = resources.flatMap((resource) => resource.operations
    .map((operation) => ({ contractId: resource.contractId, source: "contract-declared", ...operation })));
  const apis = (observedApis.length > 0 ? observedApis : declaredApis)
  ...
  const relations = resources.flatMap((resource) => resource.relations
    .map((relation) => ({ fromContractId: resource.contractId, ...relation })));
  const base = {
    schemaVersion: 1,
    ...
    diagnostics: { errors: contractErrors, warnings: contractWarnings },
...
function duplicateDiagnostics(catalogs) {
  ...
  for (const [moduleId, catalog] of Object.entries(catalogs)) {
    for (const resource of catalog.resources || []) {
      ...
      if (resource.database && resource.codegenSafe) add("migration", `${resource.database.migrationLocation}|${resource.database.migrationVersion}`, moduleId, resource.contractId, resource.database.phase);
      ...
      for (const code of resource.errorCodes || []) add("error-code", code, moduleId, resource.contractId, resource.contractFile);
      for (const id of resource.logicalIds || []) add("logical-id", id, moduleId, resource.contractId, resource.contractFile);
      for (const id of resource.integrationIds || []) add("integration-id", id, moduleId, resource.contractId, resource.contractFile);
    }
    for (const api of catalog.apis || []) {
      if (api.source !== "source-observed") continue;
      add("api", `${api.method} ${api.externalPath}`, moduleId, api.contractId, `${api.file}:${api.line}`);
    }
  }
...
function summarizeCatalog(value) {
  ...
  const stats = value.stats || {
    ...
    apis: Array.isArray(value.apis) ? value.apis.length : 0,
    databases: Array.isArray(value.databases) ? value.databases.length : 0,
    relations: Array.isArray(value.relations) ? value.relations.length : 0,
  };
  return {
    schemaVersion: value.schemaVersion,
    kind: value.kind,
    project: value.project && value.project.id ? value.project.id : value.project,
    module: value.module && value.module.id ? value.module.id : value.module,
    sourceHash: value.sourceHash,
    catalogHash: value.catalogHash,
    stats,
    contractKinds: Object.fromEntries([...new Set(resources.map((item) => item.contractKind || "legacy"))].sort()
      .map((kind) => [kind, resources.filter((item) => (item.contractKind || "legacy") === kind).length])),
    modules: modules.map((item) => ({ id: item.id, status: item.status, stats: item.stats })),
    diagnostics: {
      errors: value.diagnostics && Array.isArray(value.diagnostics.errors) ? value.diagnostics.errors.length : 0,
      warnings: value.diagnostics && Array.isArray(value.diagnostics.warnings) ? value.diagnostics.warnings.length : 0,
    },
    evidenceFiles: Array.isArray(value.sourceEvidence) ? value.sourceEvidence.length : 0,
  };
}

function catalogSlice(value, section, options = {}) {
  const allowed = new Set(["resources", "services", "apis", "databases", "relations", "sourceEvidence"]);
  if (!allowed.has(section)) return { ok: false, errors: [error("$.section", `不支持的目录分区：${section}`)] };
  ...
  const source = Array.isArray(value && value[section]) ? value[section] : [];
  const items = source.slice(cursor, cursor + limit);
  return {
    ok: true,
    catalog: summarizeCatalog(value),
    section,
    cursor,
    limit,
    total: source.length,
...
function publicFreshnessResult(result, options = {}) {
  ...
  const summary = {
    ...
    catalogSummary: summarizeCatalog(result.catalog),
    ...(result.errors ? { errors: result.errors } : {}),
  };
  return options.detail === "full" ? { ...summary, catalog: result.catalog, config: result.config } : summary;
}
