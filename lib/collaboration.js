...
function completionFor(contract, implementedOperations = []) {
  ...
  for (const operation of contract.customOperations || []) {
    ...
    if ((!hasEntityTarget || !(operation.patch || []).length) && !implemented.has(operation.name)) {
      skeletonOperations.push(operation.name);
    }
  }
  return {
...
function buildManifest(contract, profile, deliveryProfile = DEFAULT_DELIVERY_PROFILE, options = {}) {
  ...
  for (const op of contract.customOperations || []) {
    ...
    if (op.kind === "batch") {
      models[`custom_${op.name}_request`].push({ name: "ids", description: "主键集合", required: true, type: "array", items: { type: "string" } });
    }
