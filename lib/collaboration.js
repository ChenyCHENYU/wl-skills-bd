...
function contractField(field, required, table) {
  ...
  return stable({
    ...
    ...(Object.keys(constraints).length > 0 ? {
      constraints,
      constraintSource: `db-contract:${table}.${field.column}`,
    } : {}),
...
function buildManifest(contract, profile, deliveryProfile = DEFAULT_DELIVERY_PROFILE, options = {}) {
  ...
  const models = {
    pageRequest: [
      ...
      ...queryFields.map((field) => contractField(field, false, contract.entity.table)),
    ],
    createRequest: writableFields.map((field) => contractField(field, field.requiredOnCreate, contract.entity.table)),
    updateRequest: [
      syntheticField("id", "主键ID", "string", true),
      syntheticField("revision", "乐观锁版本号", "integer", true, "int32"),
      ...writableFields.map((field) => contractField(field, false, contract.entity.table)),
    ],
    detailResponse: [
      syntheticField("id", "主键ID", "string", true),
      syntheticField("revision", "乐观锁版本号", "integer", true, "int32"),
      ...detailFields.map((field) => contractField(field, false, contract.entity.table)),
    ],
    pageResponse: [
      syntheticField("id", "主键ID", "string", true),
      ...listFields.map((field) => contractField(field, false, contract.entity.table)),
    ],
  };
