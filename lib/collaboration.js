...
const { resolveProfilePolicies } = require("./profile-policy");
...
function decimalConstraints(dbType) {
  const match = String(dbType || "").match(/^DECIMAL\s*\(\s*(\d+)\s*,\s*(\d+)\s*\)$/i);
  if (!match) return {};
  return { totalDigits: Number(match[1]), fractionDigits: Number(match[2]) };
}

function varcharConstraints(field) {
  const match = String(field.dbType || "").match(/^VARCHAR2?\s*\(\s*(\d+)/i);
  return match && field.javaType === "String" ? { maxLength: Number(match[1]) } : {};
}

function storageConstraints(field) {
  return {
    ...varcharConstraints(field),
    ...decimalConstraints(field.dbType),
  };
}

function writeConstraints(field) {
  return {
    ...storageConstraints(field),
    ...(field.maxLength ? { maxLength: field.maxLength } : {}),
    ...(field.constraints || {}),
  };
}

function constraintsFor(field, operation) {
  if (operation === "page") return { ...(field.queryConstraints || {}) };
  if (["create", "update"].includes(operation)) return writeConstraints(field);
  return storageConstraints(field);
}

function constraintSourceFor(field, operation, table) {
  if (operation === "page") return field.queryConstraintSource;
  const sources = [`db-contract:${table}.${field.column}`];
  if (field.constraintSource) sources.push(field.constraintSource);
  return sources.join("; ");
}

function customOperationConstraints(field) {
  return {
    ...(field.maxLength ? { maxLength: field.maxLength } : {}),
    ...(field.constraints || {}),
  };
...
function contractField(field, required, table, operation) {
  const constraints = constraintsFor(field, operation);
  const constraintSource = constraintSourceFor(field, operation, table);
  return stable({
    name: field.name,
    ...
    ...(field.semanticId ? { "x-semantic-id": field.semanticId } : {}),
    ...(field.enumValues ? { enum: field.enumValues } : {}),
    ...(field.classification ? { "x-data-classification": field.classification } : {}),
    ...(field.masking ? { "x-masking": field.masking } : {}),
  });
}

function customRequestField(field) {
  const constraints = customOperationConstraints(field);
  return stable({
    name: field.name,
    description: field.comment,
    ...
    ...(Object.keys(constraints).length > 0 ? {
      constraints,
      constraintSource: `backend-contract:customOperations.${field.name}`,
    } : {}),
    ...(field.classification ? { "x-data-classification": field.classification } : {}),
    ...(field.masking ? { "x-masking": field.masking } : {}),
  });
...
function syntheticField(name, description, type, required, format, extras = {}) {
  return stable({ name, description, required, type, ...(format ? { format } : {}), ...extras });
}

function standardOperation(name, contract, deliveryProfile, basePaths) {
  const defaults = deliveryProfile.transport.operations[name];
  const requestModels = {
    page: "pageRequest",
    detail: "idPath",
...
function completionFor(contract, implementedOperations = [], assuranceMissing = []) {
  ...
  const assuranceQuestions = assuranceMissing.map((name) => `生产保障证据缺失或不可读：${name}`);
  return {
    contractStatus: skeletonOperations.length === 0 && assuranceQuestions.length === 0 ? "confirmed" : "draft",
    openQuestions: [
      ...skeletonOperations.map((name) => `${name} 仍是可编译骨架，必须补充业务实现与测试`),
      ...assuranceQuestions,
    ],
    deviations: [],
    skeletonOperations,
...
function buildManifest(contract, profile, deliveryProfile = DEFAULT_DELIVERY_PROFILE, options = {}) {
  ...
  const idField = () => syntheticField("id", "主键ID", "string", true, undefined, {
    constraints: { maxLength: idPolicy.maxLength },
    constraintSource: `profile:${profile.id}.idPolicy`,
    "x-id-format": idPolicy.format,
  });
  ...
  const pagination = deliveryProfile.transport.pagination;
  const operations = Object.fromEntries(STANDARD_OPERATIONS.map((name) => [
    name,
    standardOperation(name, contract, deliveryProfile, basePaths),
  ]));
  ...
  const models = {
    pageRequest: [
      syntheticField(pagination.requestCurrent, "当前页码", "integer", true, "int64", {
        default: pagination.defaultCurrent,
        constraints: { minimum: 1, totalDigits: 10, fractionDigits: 0 },
        constraintSource: "delivery-profile:transport.pagination",
      }),
      syntheticField(pagination.requestSize, "每页记录条数", "integer", true, "int64", {
        default: pagination.defaultSize,
        constraints: {
          minimum: 1,
          maximum: pagination.maxSize,
          totalDigits: 10,
          fractionDigits: 0,
        },
        constraintSource: "delivery-profile:transport.pagination",
      }),
      ...queryFields.map((field) => contractField(field, field.requiredOnQuery, contract.entity.table, "page")),
    ],
    createRequest: writableFields.map((field) => contractField(field, field.requiredOnCreate, contract.entity.table, "create")),
    updateRequest: [
      idField(),
      syntheticField("revision", "乐观锁版本号", "integer", true, "int32"),
      ...writableFields.map((field) => contractField(field, false, contract.entity.table, "update")),
    ],
    detailResponse: [
      idField(),
      syntheticField("revision", "乐观锁版本号", "integer", true, "int32"),
      ...detailFields.map((field) => contractField(field, false, contract.entity.table, "response")),
    ],
    pageResponse: [
      idField(),
      ...listFields.map((field) => contractField(field, false, contract.entity.table, "response")),
    ],
  };
  ...
  for (const op of contract.customOperations || []) {
    ...
    if (op.kind === "batch") {
      ...
      models[`custom_${op.name}_request`].push(idField());
    }
  ...
  const apiSurface = stable({
    ...
    transport: {
      ...
      pagination: {
        requestCurrent: pagination.requestCurrent,
        requestSize: pagination.requestSize,
    ...
    ...((contract.validationRules || []).length > 0 ? { validationRules: contract.validationRules } : {}),
    models,
    errors: contract.errors || [],
    assurance: contract.assurance || { level: "standard" },
  });
  ...
  return stable({
    ...
    completion: completionFor(contract, options.implementedOperations, options.assuranceMissing || []),
  });
}
...
function markdownConstraints(field) {
  const values = [];
  if (field.default !== undefined) values.push(`default=${field.default}`);
  for (const [name, value] of Object.entries(field.constraints || {})) {
...
function fieldTable(fields) {
  const lines = ["| 字段 | 类型 | 必填 | 说明 | 边界 | 来源 |", "|---|---|---|---|---|---|"];
  for (const field of fields) {
    lines.push(`| ${field.name} | ${markdownType(field)} | ${field.required ? "是" : "否"} | ${field.description} | ${markdownConstraints(field)} | ${field.constraintSource || "-"} |`);
  }
  return lines.join("\n");
}
...
function renderMarkdown(manifest) {
  ...
  const assurance = manifest.assurance || { level: "standard" };
  const assuranceSection = assurance.level === "production"
    ? [
      "",
      "## 生产保障契约",
      "",
      `- 关键度：${assurance.criticality}`,
      `- SLO：可用性 ${assurance.slo.availabilityPercent}%｜P95 ${assurance.slo.p95LatencyMs}ms｜P99 ${assurance.slo.p99LatencyMs}ms｜最大错误率 ${assurance.slo.maxErrorRatePercent}%`,
      `- 恢复目标：RTO ${assurance.recovery.rtoMinutes} 分钟｜RPO ${assurance.recovery.rpoMinutes} 分钟`,
      `- 授权：${assurance.security.authorizationModel}｜方法安全：必须｜敏感写审计：必须`,
      `- 数据责任：${assurance.dataGovernance.owner}｜事实来源：${assurance.dataGovernance.sourceOfTruth}`,
      `- 一致性：幂等 ${assurance.consistency.idempotencyStrategy}｜事件 ${assurance.consistency.eventDelivery}｜跨服务事务 ${assurance.consistency.crossServiceTransaction}`,
      `- 韧性：依赖超时 ${assurance.resilience.dependencyTimeoutMs}ms｜重试 ${assurance.resilience.retryMaxAttempts} 次｜熔断 ${assurance.resilience.circuitBreakerRequired ? "需要" : "不需要"}｜限流 ${assurance.resilience.rateLimitRequired ? "需要" : "不需要"}`,
      `- 证据：${Object.values(assurance.evidence).join("、")}`,
      "",
      "> completion 只有在上述证据文件存在且业务骨架均已实现、测试后才能 confirmed。",
    ]
    : ["", "## 生产保障契约", "", "当前为 `standard`；进入生产前应升级为 `production` 并补齐 SLO、安全、数据治理、压测、运行手册和恢复演练证据。"];
...
function generatedArtifacts(contract, profile, deliveryProfile, options = {}) {
  const manifest = buildManifest(contract, profile, deliveryProfile, options);
...
function compareFieldModels(expected, actual, errors) {
  for (const [modelName, expectedFields] of Object.entries(expected.models)) {
    ...
    for (const expectedField of expectedFields) {
      ...
      for (const key of [
        "description", "type", "format", "required", "default", "constraints", "constraintSource",
        "enum", "maxItems", "items", "x-semantic-id", "x-data-classification", "x-masking", "x-id-format",
      ]) {
        const expectedValue = expectedField[key] ?? null;
        const actualValue = actualField[key] ?? null;
        if (JSON.stringify(actualValue) !== JSON.stringify(expectedValue)) {
          issue(errors, "C106", `models.${modelName}.${expectedField.name}.${key}`, "前后端字段契约不一致", expectedValue, actualValue);
        }
      }
    }
...
function compareManifest(expected, actual, options = {}) {
  ...
  if (!actual || typeof actual !== "object") issue(errors, "C100", "$", "前端契约必须是对象");
    ...
    if (JSON.stringify(actual.errors || []) !== JSON.stringify(expected.errors || [])) {
      issue(errors, "C116", "errors", "前后端错误码契约不一致", expected.errors || [], actual.errors || []);
    }
...
function compareSchemaFields(document, schema, expectedFields, location, errors, warnings) {
  ...
  for (const field of expectedFields) {
    ...
    compareOpenApiConstraints(field, actual, `${location}.${field.name}`, errors);
...
function compareOpenApiConstraints(field, actual, location, errors) {
  const mappings = {
    minLength: "minLength",
    maxLength: "maxLength",
    pattern: "pattern",
  ...
  for (const [constraintName, openApiName] of Object.entries(mappings)) {
    const expectedValue = field.constraints?.[constraintName];
    if (expectedValue === undefined) continue;
    const actualValue = actual[openApiName];
    if (JSON.stringify(actualValue) === JSON.stringify(expectedValue)) continue;
    issue(errors, "C207", `${location}.${openApiName}`, "OpenAPI 字段边界与机器契约不一致", expectedValue, actualValue ?? null);
  }
}
...
function compareOpenApi(manifest, document) {
  ...
  for (const name of order) {
    ...
    if (["page", "create", "update"].includes(name)) {
      ...
      const expectedModel = manifest.models[expected.requestModel];
