...
const { detectTask } = require("./task-router");
const {
...
function lexicalTokens(value) {
  ...
  for (const chunk of normalized.split(/[\s,，。；;、:/\\|()[\]{}<>_-]+/u).filter(Boolean)) {
    result.push(chunk);
    for (const word of chunk.match(/[a-z0-9]+/g) || []) result.push(word);
    for (const sequence of chunk.match(/[\p{Script=Han}]+/gu) || []) {
      for (let width = 2; width <= Math.min(4, sequence.length); width += 1) {
        for (let index = 0; index + width <= sequence.length; index += 1) result.push(sequence.slice(index, index + width));
      }
    }
  }
  return result;
...
function tokenize(task, keywords) {
  ...
  const routed = detectTask(String(task || ""));
  const parts = [String(task || ""), ...explicit, ...(routed ? [routed.task.id, routed.task.name] : [])]
    .flatMap(lexicalTokens)
...
function buildContextPlan(projectRootInput, options = {}) {
  ...
  const maxTokens = boundedInteger(options.maxTokens, DEFAULT_MAX_TOKENS, 512, 200000);
  if (maxHops === null || maxFiles === null || maxBytes === null || maxTokens === null) {
    return { ok: false, projectRoot, reason: "invalid-limit", errors: [{ path: "$.limits", message: "maxHops 仅允许 0~1；maxFiles 允许 3~200；maxBytes 允许 16KiB~10MiB；maxTokens 允许 512~200000" }] };
  }
  ...
  for (const candidate of ordered) {
    ...
    if (selected.length >= maxFiles
      || selectedBytes + candidate.bytes > maxBytes
      || selectedTokens + candidate.estimatedTokens > maxTokens) omitted.push(publicCandidate);
    else {
  ...
  const omittedAuthority = omitted.find((item) => item.role === "target-catalog");
  if (omittedAuthority) {
    return {
      ok: false,
      projectRoot,
      module: moduleId,
      reason: "context-budget-too-small",
      errors: [{
        path: "$.limits",
        message: `token/字节预算不足以容纳当前模块权威目录 ${omittedAuthority.rel}；至少需要 ${omittedAuthority.estimatedTokens} tokens / ${omittedAuthority.bytes} bytes`,
      }],
      minimumBudget: { maxTokens: omittedAuthority.estimatedTokens, maxBytes: omittedAuthority.bytes },
    };
  }
  ...
  const base = {
    ...
    limits: { maxFiles, maxBytes, maxTokens },
    selection: {
      files: selected,
      ...
      omittedTokens: omitted.reduce((total, item) => total + item.estimatedTokens, 0),
    },
    linked,
    diagnostics: {
      errors: [],
      warnings: [
        ...
        ...omitted.filter((item) => item.role === "target-doc").map((item) => ({
          code: "CTX_AUTHORITY_OMITTED_BY_BUDGET",
          file: item.rel,
          message: `权威入口 ${item.rel} 超出上下文预算，建议提高 maxTokens/maxBytes 或先压缩该文件`,
        })),
      ],
    },
  };
