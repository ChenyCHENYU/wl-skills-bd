...
function checkController(content, relFile, issues) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // B2: 有 HTTP 映射但缺 @Operation/@ApiOperation（文档）
    // B2 兼容 OpenAPI 3（@Operation）和 Springfox 2（@ApiOperation 存量）
    if (!/@Operation\b|@ApiOperation\b/.test(annotWindow)) {
      pushIssue(
        issues,
        "B2",
        "warn",
        ...
        reportLine,
        1,
        "Controller 接口方法缺 @Operation（OpenAPI 3）或 @ApiOperation（Swagger 文档缺失）",
        "04/13",
      );
    }
  }
...
/** B12: Service/Mapper 方法缺 Javadoc（standards/19 §9 必要注释兜底）
 *  对 Service 的业务方法（save/update/delete/状态变更）和 Mapper 接口方法强制 Javadoc。
 *  Controller 方法已有 @ApiOperation 豁免；getter/setter 豁免。
 */
function checkMethodJavadoc(content, relFile, issues) {
  const lines = content.split(/\r?\n/);
  const isMapperInterface = /^\s*public\s+interface\s+\w*Mapper\b/m.test(content);
  const isService = /class\s+\w*(?:ServiceImpl|Service)\b/.test(content);

  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    // 定位 public 方法签名（Service 写方法 或 Mapper 接口方法）
    let methodMatch = null;
    if (isService) {
      // Service 写方法：含写动词或状态变更动词
      methodMatch = line.match(
        /^\s*public\s+\w[\w<>\[\],?\s.]*\s+(save|insert|add|update|delete|remove|submit|approve|reject|enable|disable|import|export|sync|batch\w*)\s*\(/i,
      );
    } else if (isMapperInterface) {
      // Mapper 接口：所有自定义 public 方法（排除 default）
      methodMatch = line.match(/^\s+(?!default\b)(?!static\b)[\w<>\[\],?\s]+?\s+(\w+)\s*\([^)]*\)\s*;/);
    }
    if (!methodMatch) continue;

    // 往上找 6 行注解区，判断是否有 Javadoc（/** */）
    const window = lines.slice(Math.max(0, i - 6), i).join("\n");
    const hasJavadoc = /\/\*\*[\s\S]*?\*\//.test(window);
    if (!hasJavadoc) {
      ...
      pushIssue(
        ...
        i + 1,
        1,
        `${isMapperInterface ? "接口" : "业务"}方法 ${methodName}() 缺 Javadoc（standards/19 §9：业务/接口方法需说明业务规则 + @param/@return/@throws）`,
        "15/19",
      );
    }
  }
...
function runBeRules(targetDir, options = {}) {
  ...
  for (const abs of files) {
    ...
    if (rel.endsWith("Controller.java")) {
      ...
      if (!isExempt(rel, "B12")) checkMethodJavadoc(content, rel, issues);
    } else if (rel.endsWith("Mapper.java")) {
      // Mapper 接口：查 B12 接口方法 Javadoc
      if (!isExempt(rel, "B12")) checkMethodJavadoc(content, rel, issues);
