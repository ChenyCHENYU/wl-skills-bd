...
const { discoverWorkspace, prefixRelative, relativeToModule } = require("./workspace");
...
function checkPhysicalDelete(content, relFile, output, softDelete) {
  ...
  while ((match = PHYSICAL_DELETE_REGEX.exec(scrubbed)) !== null) {
    const pos = lineAt(offsets, match.index);
    output.push(issue("B17", relFile, pos.line, pos.col, `检测到 ${match[1]}()：业务代码禁止物理删除，团队基线软删 DELETE_FLAG=${deletedValue}；物理删须走独立运维契约 + DBA 双签`));
  }
  ...
  while ((match = JAVA_PHYSICAL_DELETE_REGEX.exec(content)) !== null) {
    const pos = lineAt(offsets, match.index);
    output.push(issue("B17", relFile, pos.line, pos.col, `检测到 Java SQL 物理 DELETE FROM；业务数据必须使用 DELETE_FLAG=${deletedValue} 原子软删除`));
  }
}
...
function checkWrapperWriteSafety(content, relFile, output, tenantMode) {
  ...
  while ((match = wrappers.exec(scrubbed)) !== null) {
    ...
    if (!/getDeleteFlag\b|\bDELETE_FLAG\b/.test(match[0])) missing.push("DELETE_FLAG");
    if (!/getRevision\b|\bREVISION\b/.test(match[0])) missing.push("revision");
    if (missing.length === 0) continue;
    const pos = lineAt(offsets, match.index);
    output.push(issue("B18", relFile, pos.line, pos.col, `Wrapper 写入缺少 ${missing.join("/")} 约束；受管写必须使用 ID + COMPANY_ID + DELETE_FLAG + REVISION 原子 SQL`));
  }
}
...
function checkJacksonTransportDto(content, relFile, output) {
  if (/(?:^|\/)src\/test\//.test(relFile)) return;
  const fileName = path.basename(relFile);
  const isTransportPath = /(?:^|\/)dto\/(?:request|response)(?:\/|$)/i.test(relFile);
  const isTransportName = /(?:Request|Response|Result)(?:DTO|Dto|VO)\.java$/.test(fileName);
  if (!isTransportPath && !isTransportName) return;

  const scrubbed = stripJava(content);
  const classMatch = /\bclass\s+([A-Z]\w*)\b/.exec(scrubbed);
  if (!classMatch || /\babstract\s+class\b/.test(scrubbed)) return;
  const className = classMatch[1];
  if (/@JsonCreator\b|@Jacksonized\b|@JsonDeserialize\s*\([^)]*builder\s*=/s.test(content)) return;

  const escapedClassName = className.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const constructorRegex = new RegExp(`(?:public|protected|private)?\\s*${escapedClassName}\\s*\\(([^)]*)\\)`, "g");
  let constructor;
  let hasNoArgsConstructor = /@NoArgsConstructor\b/.test(content);
  let hasParameterizedConstructor = false;
  while ((constructor = constructorRegex.exec(scrubbed)) !== null) {
    if (constructor[1].trim()) hasParameterizedConstructor = true;
    else hasNoArgsConstructor = true;
  }

  const hasFinalField = /\bprivate\s+final\s+[\w<>,?.\[\]]+\s+\w+\s*(?:=[^;]+)?;/.test(scrubbed);
  const lombokConstructor = /@(AllArgsConstructor|RequiredArgsConstructor|Value|Builder)\b/.exec(content);
  const dataRequiredConstructor = /@Data\b/.test(content) && (hasFinalField || /@NonNull\b/.test(content));
  const removesImplicitNoArgs = Boolean(lombokConstructor) || dataRequiredConstructor || hasParameterizedConstructor;
  if (!removesImplicitNoArgs || hasNoArgsConstructor) return;

  const marker = lombokConstructor || classMatch;
  const pos = lineAt(lineOffsets(content), marker.index);
  output.push(issue(
    "B32",
    relFile,
    pos.line,
    pos.col,
    `${className} 是跨服务 JSON DTO，但只有有参构造且缺少可反序列化入口；使用“非 final 字段 + @NoArgsConstructor + setter”，或为不可变对象逐参数声明 @JsonCreator/@JsonProperty，并补 ObjectMapper 往返测试`,
  ));
}
...
function checkUpdateDeleteWithoutWhere(content, relFile, output, tenantMode = "explicit", softDelete, enabled = () => true) {
  ...
  while ((match = updateOrDeleteTags.exec(executable)) !== null) {
    ...
    if (enabled("B17") && tag.toLowerCase() === "delete") {
      const position = lineAt(offsets, match.index);
      const deletedValue = softDelete && Number.isInteger(softDelete.deletedValue) ? softDelete.deletedValue : 0;
      output.push(issue("B17", relFile, position.line, position.col, `Mapper XML <delete> 是物理删除；业务数据必须 UPDATE DELETE_FLAG=${deletedValue}`));
    }
  }
}
...
function runBeRulesSingle(targetDirInput, options = {}) {
  ...
  for (const [rel, content] of contents) {
    ...
    if (rel.endsWith(".java")) {
      ...
      if (executionPlan.groupEnabled("jacksonTransportDto")) checkJacksonTransportDto(content, rel, rawIssues);
  ...
  if (executionPlan.groupEnabled("databaseSource")) {
    ...
    const hasDbSpec = fs.existsSync(path.join(targetDir, "docs", "db-spec"));
    if (contractTables.size > 0 || hasDbSpec) {
...
function prefixFinding(value, module) {
  return {
    ...value,
    file: prefixRelative(module.rel, value.file),
    module: module.id,
  };
}

function runWorkspaceRules(targetDir, options, workspace) {
  const selectedModules = options.module
    ? workspace.modules.filter((module) => module.id === options.module)
    : workspace.modules;
  if (selectedModules.length === 0) {
    const executionPlan = createRuleExecutionPlan({ rules: options.rules, quick: options.quick === true, staged: Array.isArray(options.stagedFiles) });
    const value = issue("WLS_CONFIG", ".wl-skills-bd/catalog.config.json", 1, 1, `未知模块：${options.module}`);
    return {
      schemaVersion: 1,
      targetDir,
      workspace: true,
      ...
      issues: [value],
      suppressed: [],
      stats: { error: 1, warn: 0, info: 0, total: 1, suppressed: 0, byRule: { WLS_CONFIG: 1 } },
      status: executionPlan.coverage.status,
      coverage: executionPlan.coverage,
      evaluatedRules: executionPlan.evaluatedRules,
      skippedRules: executionPlan.skippedRules,
      modules: [],
      execution: {
        requestedRules: executionPlan.requestedRules,
        executedRules: executionPlan.evaluatedRules,
        executedGroups: executionPlan.groups,
        skippedRules: executionPlan.skippedRules,
        unknownRules: executionPlan.unknownRules,
        scan: { discoveredFiles: 0, loadedFiles: 0, loadedBytes: 0, oversizedFiles: 0, contentCacheHits: 0, contentCacheMisses: 0 },
        workspace: { configHash: workspace.configHash, modules: [] },
      },
      durationMs: 0,
    };
  }
  ...
  for (const module of selectedModules) {
    ...
    if (Array.isArray(options.stagedFiles)) {
      stagedFiles = options.stagedFiles
        .map((file) => relativeToModule(module, file))
        .filter((file) => file !== null);
      if (stagedFiles.length === 0) continue;
    }
    const result = runBeRulesSingle(module.root, {
      ...options,
      stagedFiles,
      workspace: false,
  ...
  const issues = moduleResults.flatMap(({ module, result }) => result.issues.map((value) => prefixFinding(value, module)));
  const suppressed = moduleResults.flatMap(({ module, result }) => result.suppressed.map((value) => prefixFinding(value, module)));
  const endpoints = moduleResults.flatMap(({ module, result }) => result.endpoints.map((value) => ({
    ...value,
    ...(value.file ? { file: prefixRelative(module.rel, value.file) } : {}),
    module: module.id,
  })));
  issues.sort((left, right) => left.file.localeCompare(right.file) || left.line - right.line || left.rule.localeCompare(right.rule));
  suppressed.sort((left, right) => left.file.localeCompare(right.file) || left.line - right.line || left.rule.localeCompare(right.rule));
  const stats = {
    error: issues.filter((value) => value.severity === "error").length,
    warn: issues.filter((value) => value.severity === "warn").length,
    info: issues.filter((value) => value.severity === "info").length,
  ...
  const first = moduleResults[0] && moduleResults[0].result;
  const scan = moduleResults.reduce((acc, { result }) => {
    for (const [key, value] of Object.entries(result.execution.scan || {})) acc[key] = (acc[key] || 0) + Number(value || 0);
    return acc;
  }, {});
  return {
    schemaVersion: 1,
    ...
    suppressed,
    stats,
    status: first ? first.status : "complete",
    coverage: first ? first.coverage : { status: "complete" },
    evaluatedRules: first ? first.evaluatedRules : [],
    skippedRules: first ? first.skippedRules : [],
    modules: moduleResults.map(({ module, result }) => ({ id: module.id, root: module.rel, stats: result.stats, execution: result.execution })),
    execution: {
      ...(first ? first.execution : {}),
      workspace: { configHash: workspace.configHash, modules: moduleResults.map(({ module }) => module.id) },
      scan,
    },
    durationMs: moduleResults.reduce((sum, { result }) => sum + result.durationMs, 0),
  };
}

function runBeRules(targetDirInput, options = {}) {
  const targetDir = path.resolve(targetDirInput);
  if (options.workspace !== false && !options.scanRel) {
    const workspace = discoverWorkspace(targetDir);
    if (workspace.enabled) return runWorkspaceRules(targetDir, options, workspace);
  }
  return runBeRulesSingle(targetDir, options);
}

module.exports = {
  DEFAULT_THRESHOLDS,
  checkHttpNoTimeout,
  checkJacksonTransportDto,
  checkPhysicalDelete,
