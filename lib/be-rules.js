...
// 由统一 Source Index 汇聚契约表/字段，兼容旧版 docs/contracts/db 布局。
function collectContractTables(projectRoot) {
  return sourceIndex.collectContractTables(projectRoot).tables;
}
...
function runBeRules(targetDirInput, options = {}) {
  ...
  if (options.quick) {
    for (const rule of ["B9", "B10", "B11", "B12"]) skippedReasons.set(rule, "quick 模式跳过设计级检查");
  }
  if (stagedMode) {
  ...
  const skippedRules = selectedRuleIds
    .filter((rule) => skippedReasons.has(rule))
    .map((rule) => ({ rule, reason: skippedReasons.get(rule) }));
  const evaluatedRules = selectedRuleIds.filter((rule) => !skippedReasons.has(rule));
  const coverage = {
    status: skippedRules.length > 0 ? "partial" : "complete",
    mode: stagedMode ? "staged" : options.quick ? "quick" : "full",
    evaluatedRules,
    skippedRules,
  };
  return {
