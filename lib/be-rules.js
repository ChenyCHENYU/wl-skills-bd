...
const SCANNED_EXTENSIONS = new Set([".java", ".xml", ".yml", ".yaml", ".properties"]);
...
function checkMapperXml(content, relFile, output, tenantMode, softDelete) {
...
function checkUpdateDeleteWithoutWhere(content, relFile, output, tenantMode = "explicit", softDelete) {
  const executable = stripXmlComments(content);
  const offsets = lineOffsets(executable);
  ...
  while ((match = updateOrDeleteTags.exec(executable)) !== null) {
    ...
    if (invalidWhere || dynamicOnly || missingTenant) {
      const position = lineAt(offsets, match.index);
      const causes = [
...
function runBeRules(targetDirInput, options = {}) {
  ...
  for (const absolute of files) {
    ...
    if (rel.endsWith(".xml") && /(?:^|\/)mapper(?:\/|$)|Mapper\.xml$/i.test(rel)) {
      checkMapperXml(content, rel, rawIssues, config.tenant.mode, config.softDelete);
      checkUpdateDeleteWithoutWhere(content, rel, rawIssues, config.tenant.mode, config.softDelete);
    }
    ...
    if (rel.endsWith(".java")) {
      ...
      checkPhysicalDelete(content, rel, rawIssues, config.softDelete);
      ...
      checkJavaTenantAccess(content, rel, rawIssues, config.tenant.mode);
      checkWrapperWriteSafety(content, rel, rawIssues, config.tenant.mode);
      checkSensitiveToString(content, rel, rawIssues);
    }
  ...
  checkMethodSecurityActivation(contents, rawIssues);
  if (!stagedMode) {
    checkMapperDiscoverability(contents, rawIssues);
    checkMapperResourceLocations(contents, rawIssues);
  }
  checkParentManagedDependencyVersions(contents, rawIssues);
  checkFrameworkExtensionBeanSafety(contents, rawIssues);
  checkPageDtoPagination(targetDir, contents, rawIssues);
  const endpoints = collectControllerEndpoints(contents);
  checkDuplicateControllerEndpoints(endpoints, rawIssues);
  if (!stagedMode) checkDirectoryDensity(targetDir, files, rawIssues, config.thresholds);
  ...
  const selectedRuleIds = [...RULES.keys()]
    .filter((rule) => !ruleSet || ruleSet.has(rule))
    .sort((left, right) => Number(left.slice(1)) - Number(right.slice(1)));
  const skippedReasons = new Map();
  if (options.quick) for (const rule of ["B9", "B10", "B11", "B12"]) skippedReasons.set(rule, "quick 模式跳过设计级检查");
  if (stagedMode) for (const [rule, reason] of [
  ...
  ]) skippedReasons.set(rule, reason);
  const skippedRules = selectedRuleIds.filter((rule) => skippedReasons.has(rule)).map((rule) => ({ rule, reason: skippedReasons.get(rule) }));
  const evaluatedRules = selectedRuleIds.filter((rule) => !skippedReasons.has(rule));
  const coverage = { status: skippedRules.length > 0 ? "partial" : "complete", mode: stagedMode ? "staged" : options.quick ? "quick" : "full", evaluatedRules, skippedRules };
  return { schemaVersion: 1, targetDir, endpoints, issues, suppressed, stats, status: coverage.status, coverage, evaluatedRules, skippedRules, durationMs: Date.now() - started };
}
