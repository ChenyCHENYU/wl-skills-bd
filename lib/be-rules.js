...
// 汇聚 docs/contracts/db 下各契约的物理表描述（B31 用）
function collectContractTables(projectRoot) {
  const dir = path.join(projectRoot, "docs", "contracts", "db");
  ...
  for (const entry of entries) {
    ...
    tables.set(String(table).toLowerCase(), dbSpec.descriptorFromContract(parsed, normalizeRel(path.relative(projectRoot, file))));
  }
  return tables;
}
...
function runBeRules(targetDirInput, options = {}) {
  ...
  if (!options.stagedFiles && !options.quick) {
    const dbSpec = require("./db-spec");
  ...
  return { schemaVersion: 1, targetDir, endpoints, issues, suppressed, stats, durationMs: Date.now() - started };
}
