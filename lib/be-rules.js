...
function checkPhysicalDelete(content, relFile, output) {
  ...
  while ((match = JAVA_PHYSICAL_DELETE_REGEX.exec(content)) !== null) {
    const pos = lineAt(offsets, match.index);
    output.push(issue("B17", relFile, pos.line, pos.col, "检测到 Java SQL 物理 DELETE FROM；业务数据必须使用 IS_DELETE=0 原子软删除"));
  }
}
...
function checkUpdateDeleteWithoutWhere(content, relFile, output, tenantMode = "explicit") {
  const executable = stripXmlComments(content);
  const offsets = lineOffsets(executable);
  ...
  while ((match = updateOrDeleteTags.exec(executable)) !== null) {
    ...
    if (tag.toLowerCase() === "delete") {
      const position = lineAt(offsets, match.index);
      output.push(issue("B17", relFile, position.line, position.col, "Mapper XML <delete> 是物理删除；业务数据必须 UPDATE IS_DELETE=0"));
    }
  }
}
...
function runBeRules(targetDirInput, options = {}) {
  ...
  for (const absolute of files) {
    ...
    if (rel.endsWith(".xml") && /(?:^|\/)mapper(?:\/|$)|Mapper\.xml$/i.test(rel)) {
      ...
      checkUpdateDeleteWithoutWhere(content, rel, rawIssues, config.tenant.mode);
    }
