...
const TASK_TYPES = {
  "new-service": {
    ...
    standards: ["01", "02", "04", "05", "06", "07", "08", "10", "11", "12", "13", "28", "29"],
    ...
    steps: [
      ...
      "5. validate src/main（B1~B31，含退役表代码引用扫描）",
  ...
  "add-api": {
    ...
    standards: ["04", "05", "10", "11", "13", "28", "29"],
    ...taskRules("add-api"),
    steps: [
      ...
      "6. validate <目标文件> --rules B1,B2,B5,B8,B12,B20,B24,B25,B26,B30,B31（含数据库事实源门禁）",
...
// Pre-flight 声明不再只是自我报告：下游可对比实际读取内容与这里的哈希。
...
function locateStandards(root, id) {
  ...
  const found = fs.readdirSync(root).find((file) => file.startsWith(`${id}-`) && file.endsWith(".md"));
...
function locateSkill(root, name) {
  ...
  for (const category of fs.readdirSync(root, { withFileTypes: true })) {
    ...
    const file = path.join(root, category.name, name, "SKILL.md");
...
function buildPreflightEvidence(taskId, projectRootInput) {
  ...
  const root = installed ? path.resolve(projectRootInput) : null;
...
function formatTaskPlan(task, options = {}) {
  ...
  const lines = [
    ...
    "",
    "🔐 Pre-flight 证据（标准/skill 文件 sha256 清单，防自报）：",
