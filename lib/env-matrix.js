...
const { hashBuffer, writeJsonAtomic, normalizeRel, resolveWithin } = require("./manifest");
...
function renderK8sDeploymentTemplate(cfg) {
  ...
  return [
    ...
    "      containers:",
    `        - name: ${cfg.project}`,
    `          image: "${cfg.k8sRegistry}/${cfg.project}:${isProd ? "5" : "latest"}"`,
...
function applyMigrationPlan(plan, options = {}) {
  ...
  return { ok: true, planHash: plan.planHash, from: plan.from, to: plan.to, applied };
}
