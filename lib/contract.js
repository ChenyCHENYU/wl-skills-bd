...
const {
  databaseFlagSuggestion,
  forbiddenDatabaseField,
  forbiddenJavaField,
  javaFlagSuggestion,
  physicalIdentifier,
  resolveGovernance,
...
const { resolveProfilePolicies, validateProfilePolicies } = require("./profile-policy");
const { PACKAGE_DB_CLUSTERS: TEAM_DB_CLUSTERS } = require("./team-database");
const { validateIntegrations, validateLogicalIds } = require("./integration-contract");

const PACKAGE_ROOT = path.resolve(__dirname, "..");
const PACKAGE_FILES = path.join(PACKAGE_ROOT, "files");
const DELIVERY_PROFILE_NAME = "wl-delivery-profile.v1.json";
const RESERVED_FIELDS = new Set([
  "id", "companyId", "deleteFlag", "revision", "createUserNo", "updateUserNo",
...
const STRING_CONSTRAINT_KEYS = new Set(["minLength", "maxLength", "pattern"]);
const NUMBER_CONSTRAINT_KEYS = new Set([
  "minimum", "maximum", "minExclusive", "maxExclusive", "totalDigits", "fractionDigits",
]);
...
function validateGeneration(errors, value, location) {
  if (value === undefined) return undefined;
  if (!checkKeys(errors, value, location, ["source", "schemaSnapshot", "phase"])) return value;
  for (const key of ["source", "schemaSnapshot"]) {
    if (value[key] === undefined) continue;
    if (!checkString(errors, value[key], `${location}.${key}`, null, 300)) continue;
    const normalized = String(value[key]).replace(/\\/g, "/");
    if (path.isAbsolute(value[key]) || normalized.split("/").includes("..")) {
      fail(errors, `${location}.${key}`, "必须是安全的项目相对路径");
    }
  }
  if (value.phase !== undefined) checkString(errors, value.phase, `${location}.phase`, /^[A-Za-z0-9._-]+$/, 100);
  if (!value.source && !value.schemaSnapshot && !value.phase) fail(errors, location, "至少声明 source/schemaSnapshot/phase 之一");
  return { ...value };
}

function validateConstraints(errors, value, location, javaType) {
  if (value === undefined) return undefined;
  if (!checkKeys(errors, value, location, CONSTRAINT_KEYS) || Object.keys(value).length === 0) {
    if (object(value) && Object.keys(value).length === 0) fail(errors, location, "约束不能为空对象");
    return value;
  }
  const keys = Object.keys(value);
  const stringLike = javaType === "String" || javaType === "List<String>" || javaType === "List<Long>";
  const numberLike = ["Integer", "Long", "BigDecimal"].includes(javaType);
  if (keys.some((key) => STRING_CONSTRAINT_KEYS.has(key)) && !stringLike) {
    fail(errors, location, `minLength/maxLength/pattern 只适用于 String/List，当前为 ${javaType}`);
  }
  if (keys.some((key) => NUMBER_CONSTRAINT_KEYS.has(key)) && !numberLike) {
    fail(errors, location, `数值范围/精度约束只适用于 Integer/Long/BigDecimal，当前为 ${javaType}`);
  }
  for (const key of ["minLength", "maxLength", "totalDigits", "fractionDigits"]) {
    if (value[key] !== undefined && (!Number.isInteger(value[key]) || value[key] < (key === "totalDigits" || key === "maxLength" ? 1 : 0))) {
      fail(errors, `${location}.${key}`, "必须是合法的非负整数边界");
    }
  }
  if (value.pattern !== undefined) {
    checkString(errors, value.pattern, `${location}.pattern`, null, 500);
    try { new RegExp(value.pattern); } catch { fail(errors, `${location}.pattern`, "必须是合法正则表达式"); }
  }
  for (const key of ["minimum", "maximum"]) {
    if (value[key] !== undefined && (typeof value[key] !== "number" || !Number.isFinite(value[key]))) {
      fail(errors, `${location}.${key}`, "必须是有限数字");
    }
  }
  for (const key of ["minExclusive", "maxExclusive"]) {
    if (value[key] !== undefined && typeof value[key] !== "boolean") fail(errors, `${location}.${key}`, "必须是布尔值");
  }
  if (value.minExclusive === true && value.minimum === undefined) fail(errors, `${location}.minExclusive`, "为 true 时必须声明 minimum");
  if (value.maxExclusive === true && value.maximum === undefined) fail(errors, `${location}.maxExclusive`, "为 true 时必须声明 maximum");
  if (Number.isFinite(value.minimum) && Number.isFinite(value.maximum) && value.minimum > value.maximum) {
    fail(errors, location, "minimum 不能大于 maximum");
  }
  if (Number.isInteger(value.minLength) && Number.isInteger(value.maxLength) && value.minLength > value.maxLength) {
    fail(errors, location, "minLength 不能大于 maxLength");
  }
  if (Number.isInteger(value.totalDigits) && Number.isInteger(value.fractionDigits) && value.fractionDigits > value.totalDigits) {
    fail(errors, location, "fractionDigits 不能大于 totalDigits");
  }
  if (["Integer", "Long"].includes(javaType) && Number(value.fractionDigits || 0) !== 0) {
    fail(errors, `${location}.fractionDigits`, "整数类型的小数位必须为0");
  }
  return value;
}

function dbStorageConstraints(field) {
  const result = {};
  const varchar = String(field.dbType || "").match(/^VARCHAR2?\((\d+)/i);
  if (varchar && field.javaType === "String") result.maxLength = Number(varchar[1]);
  const decimal = String(field.dbType || "").match(/^(?:DECIMAL|NUMBER)\((\d+)(?:,(\d+))?\)$/i);
  if (decimal && ["Integer", "Long", "BigDecimal"].includes(field.javaType)) {
    result.totalDigits = Number(decimal[1]);
    result.fractionDigits = Number(decimal[2] || 0);
  }
  return result;
}

function effectiveWriteConstraints(field) {
  const storage = dbStorageConstraints(field);
  const legacy = field.maxLength ? { maxLength: field.maxLength } : {};
  return { ...storage, ...legacy, ...(field.constraints || {}) };
}

function effectiveQueryConstraints(field) {
  return { ...(field.queryConstraints || {}) };
}
...
function resolveProfile(profileId, projectRoot) {
  ...
  let profile = readJson(file);
  const localOverrideFile = path.join(projectRoot, ".wl-skills-bd", "profile.local.json");
  if (fs.existsSync(localOverrideFile)) {
    const localOverride = readJson(localOverrideFile);
    const allowedKeys = new Set(["schemaVersion", "profileId", "softDelete", "auditTime", "idPolicy", "auditColumns"]);
    const unknownKeys = Object.keys(localOverride).filter((key) => !allowedKeys.has(key));
    if (localOverride.schemaVersion !== 1 || localOverride.profileId !== profileId || unknownKeys.length > 0) {
      throw new Error(`${localOverrideFile}: profile.local.json schemaVersion/profileId/属性不合法`);
    }
    if (!localOverride.softDelete && !localOverride.auditTime && !localOverride.idPolicy && !localOverride.auditColumns) {
      throw new Error(`${localOverrideFile}: profile.local.json 至少覆盖 softDelete/auditTime/idPolicy/auditColumns 之一`);
    }
    const localValidation = validateGovernance(localOverride);
    if (!localValidation.ok) {
      throw new Error(`${localOverrideFile}: 本地治理覆盖非法：${localValidation.errors.join("；")}`);
    }
    profile = {
      ...profile,
      ...(localOverride.softDelete ? {
        softDelete: { ...(profile.softDelete || {}), ...localOverride.softDelete },
      } : {}),
      ...(localOverride.auditTime ? {
        auditTime: { ...(profile.auditTime || {}), ...localOverride.auditTime },
      } : {}),
      ...(localOverride.idPolicy ? {
        idPolicy: { ...(profile.idPolicy || {}), ...localOverride.idPolicy },
      } : {}),
      ...(localOverride.auditColumns ? {
        auditColumns: { ...(profile.auditColumns || {}), ...localOverride.auditColumns },
      } : {}),
    };
  }
  ...
  const policyValidation = validateProfilePolicies(profile);
  if (!policyValidation.ok) {
    throw new Error(`${file}: ID/审计列 profile 非法：${policyValidation.errors.join("；")}`);
  }
  ...
  for (const name of STANDARD_OPERATIONS) {
    ...
    if (!shared || !["GET", "POST", "PUT", "DELETE", "PATCH"].includes(shared.method)
      || !/^[A-Za-z][A-Za-z0-9]*(?:\/(?:[A-Za-z][A-Za-z0-9]*|\{[a-z][A-Za-z0-9]*\}))*$/.test(shared.path || "")) {
      throw new Error(`${deliveryFile}: transport.operations.${name} method/path 不合法`);
    }
    if (backend.method !== shared.method || backend.path !== shared.path) {
      profileWarnings.push(`transport.operations.${name} 使用项目交付口径 ${shared.method} ${shared.path}，不同于后端基线 ${backend.method} ${backend.path}`);
    }
  }
  ...
  if (!sharedResponse || sharedResponse.successCode !== profile.response.successCode
    ...
    || typeof sharedPagination.requestCurrent !== "string" || !sharedPagination.requestCurrent
    || typeof sharedPagination.requestSize !== "string" || !sharedPagination.requestSize
    || !Number.isInteger(sharedPagination.defaultCurrent) || sharedPagination.defaultCurrent < 1
    || !Number.isInteger(sharedPagination.defaultSize) || sharedPagination.defaultSize < 1
    || !Number.isInteger(sharedPagination.maxSize) || sharedPagination.maxSize < sharedPagination.defaultSize
    || sharedPagination.maxSize > 10000
  ...
  if (sharedPagination.defaultCurrent !== 1 || sharedPagination.defaultSize !== 10 || sharedPagination.maxSize !== 200) {
    profileWarnings.push(`transport.pagination 使用项目口径 current=${sharedPagination.defaultCurrent}, size=${sharedPagination.defaultSize}, maxSize=${sharedPagination.maxSize}；基线为 1/10/200`);
  }
  return { file, profile, deliveryFile, deliveryProfile, profileWarnings };
}

const LOGICAL_GOVERNANCE_COLUMNS = [
  "ID", "COMPANY_ID", "DELETE_FLAG", "REVISION", "CREATE_USER_NO", "UPDATE_USER_NO",
  "CREATE_DATE_TIME", "UPDATE_DATE_TIME",
];

function physicalIdentifierPattern(database) {
  return database === "mysql" ? /^[a-z][a-z0-9_]*$/ : /^[A-Z][A-Z0-9_]*$/;
}

function governanceColumns(database) {
  return new Set(LOGICAL_GOVERNANCE_COLUMNS.map((value) => physicalIdentifier(database, value)));
}
...
function validateIndexes(errors, rawIndexes, location, database, allowedColumns) {
  ...
  return rawIndexes.map((entry, index) => {
    ...
    checkString(errors, entry.name, `${loc}.name`, physicalIdentifierPattern(database));
    ...
    if (!Array.isArray(entry.columns) || entry.columns.length === 0) {
      ...
      entry.columns.forEach((col, ci) => {
        checkString(errors, col, `${loc}.columns[${ci}]`, physicalIdentifierPattern(database));
    ...
    if (entry.unique === true && entry.columns.includes(physicalIdentifier(database, "DELETE_FLAG"))) {
      fail(errors, `${loc}.columns`, "唯一索引禁止包含 DELETE_FLAG：重复创建/删除会在已删除值上冲突；默认采用业务键不复用策略");
    }
...
function validateCustomField(errors, raw, location, contractFieldNames) {
  ...
  if (forbiddenJavaField(raw.name)) {
    fail(errors, `${location}.name`, `禁止 isXxx 字段；Java Bean get/set 与序列化映射存在歧义，请改为 ${javaFlagSuggestion(raw.name)}`);
  }
...
function businessReadableMessage(errors, value, location) {
  if (!checkString(errors, value, location, null, 100)) return;
  if (/业务唯一键|唯一键|constraint|duplicate\s+key/i.test(value)) {
    fail(errors, location, "必须使用业务对象名称，禁止暴露业务唯一键/constraint/duplicate key 等技术术语");
  }
  if (!safeJavaText(value)) fail(errors, location, "不能包含换行、双引号、反斜杠或注释闭合符");
}

function validateBusinessKeys(errors, rawKeys, location, contractFields, production) {
  if (rawKeys === undefined) return [];
  if (!Array.isArray(rawKeys) || rawKeys.length === 0) {
    fail(errors, location, "必须是非空数组");
    return [];
  }
  const fieldByName = new Map(contractFields.map((field) => [field.name, field]));
  const names = new Set();
  return rawKeys.map((entry, index) => {
    const loc = `${location}[${index}]`;
    const allowed = [
      "name", "fields", "scopeFields", "operations", "normalization", "message", "source",
      "databaseConstraintRef",
    ];
    if (!checkKeys(errors, entry, loc, allowed, [
      "name", "fields", "operations", "normalization", "message", "source", "databaseConstraintRef",
    ])) return null;
    checkString(errors, entry.name, `${loc}.name`, /^[a-z][A-Za-z0-9]*$/, 50);
    if (names.has(entry.name)) fail(errors, `${loc}.name`, "业务键名称重复");
    names.add(entry.name);
    const validateFields = (values, key, required) => {
      if (!Array.isArray(values) || (required && values.length === 0)) {
        fail(errors, `${loc}.${key}`, required ? "必须是非空数组" : "必须是数组");
        return [];
      }
      if (new Set(values).size !== values.length) fail(errors, `${loc}.${key}`, "字段不能重复");
      values.forEach((field, fieldIndex) => {
        if (!fieldByName.has(field)) fail(errors, `${loc}.${key}[${fieldIndex}]`, `引用了不存在的契约字段 ${field}`);
      });
      return values;
    };
    const keyFields = validateFields(entry.fields, "fields", true);
    const scopeFields = entry.scopeFields === undefined ? [] : validateFields(entry.scopeFields, "scopeFields", false);
    if (scopeFields.some((field) => keyFields.includes(field))) fail(errors, `${loc}.scopeFields`, "不能与 fields 重复");
    if (!Array.isArray(entry.operations) || entry.operations.length === 0
      || entry.operations.some((operation) => !["create", "update"].includes(operation))) {
      fail(errors, `${loc}.operations`, "只允许 create/update 的非空数组");
    } else if (new Set(entry.operations).size !== entry.operations.length) fail(errors, `${loc}.operations`, "操作不能重复");
    if (!["exact", "trim"].includes(entry.normalization)) fail(errors, `${loc}.normalization`, "只允许 exact/trim");
    if (entry.normalization === "trim") {
      for (const field of [...keyFields, ...scopeFields]) {
        if (fieldByName.get(field)?.javaType !== "String") fail(errors, `${loc}.normalization`, `trim 只适用于 String 字段：${field}`);
      }
    }
    businessReadableMessage(errors, entry.message, `${loc}.message`);
    checkString(errors, entry.source, `${loc}.source`, null, 300);
    checkString(errors, entry.databaseConstraintRef, `${loc}.databaseConstraintRef`, null, 300);
    if (production && !entry.databaseConstraintRef) fail(errors, `${loc}.databaseConstraintRef`, "生产契约必须关联数据库并发唯一性方案");
    return {
      ...entry,
      scopeFields,
    };
...
function validateBatchPolicy(errors, value, location, fieldByName, kind) {
  if (value === undefined) return undefined;
  if (kind !== "batch") fail(errors, location, "batchPolicy 只允许用于 kind=batch");
  const allowed = ["selectionScope", "minItems", "sameFields", "distinct", "messages", "source"];
  if (!checkKeys(errors, value, location, allowed, allowed)) return value;
  if (value.selectionScope !== "selected-only") fail(errors, `${location}.selectionScope`, "必须为 selected-only，禁止校验未选中的同组记录");
  if (!Number.isInteger(value.minItems) || value.minItems < 2 || value.minItems > 1000) {
    fail(errors, `${location}.minItems`, "必须是 2~1000 的整数");
  }
  if (!Array.isArray(value.sameFields) || value.sameFields.length === 0) fail(errors, `${location}.sameFields`, "必须是非空数组");
  else value.sameFields.forEach((field, index) => {
    if (!fieldByName.has(field)) fail(errors, `${location}.sameFields[${index}]`, `引用了不存在的契约字段 ${field}`);
  });
  if (!Array.isArray(value.distinct) || value.distinct.length === 0) fail(errors, `${location}.distinct`, "必须是非空数组");
  else value.distinct.forEach((item, index) => {
    const itemLocation = `${location}.distinct[${index}]`;
    if (!checkKeys(errors, item, itemLocation, ["field", "count"], ["field", "count"])) return;
    if (!fieldByName.has(item.field)) fail(errors, `${itemLocation}.field`, `引用了不存在的契约字段 ${item.field}`);
    if (!Number.isInteger(item.count) || item.count < 1 || item.count > 1000) fail(errors, `${itemLocation}.count`, "必须是 1~1000 的整数");
  });
  if (checkKeys(errors, value.messages, `${location}.messages`, ["minItems", "sameFields", "distinct"], ["minItems", "sameFields", "distinct"])) {
    for (const key of ["minItems", "sameFields", "distinct"]) businessReadableMessage(errors, value.messages[key], `${location}.messages.${key}`);
  }
  checkString(errors, value.source, `${location}.source`, null, 300);
  return value;
}
...
function validateCustomOperations(errors, rawOps, location, contractFields, contractFieldNames, permissionPrefix) {
  ...
  return rawOps.map((op, index) => {
    const loc = `${location}[${index}]`;
    const allowed = ["name", "summary", "method", "path", "permission", "kind", "idFrom", "requestFields", "preconditions", "patch", "batchPolicy", "externalId"];
    ...
    const batchPolicy = validateBatchPolicy(errors, op.batchPolicy, `${loc}.batchPolicy`, fieldByName, op.kind);
    return { ...op, idFrom, ...(batchPolicy ? { batchPolicy } : {}) };
  }).filter(Boolean);
}

function validateRelations(errors, rawRelations, location, contractId, database) {
  ...
  return rawRelations.map((rel, index) => {
    ...
    checkString(errors, rel.joinColumn, `${loc}.joinColumn`, physicalIdentifierPattern(database));
...
/**
 * 状态机闭环校验（v0.26）：把 stateTransition 的前置/patch 组合成状态转移图，
 * 从 initialValue 出发做可达性分析。发现三类业务闭环漏洞：
 * 1. 不可达枚举值（声明了但任何转移都到不了——缺操作或冗余枚举）→ error
 * 2. 永不可触发的操作（前置状态组合在枚举内为空集）→ error
 * 3. 无终态（所有状态都有出边；循环流程可能是有意的）→ warn，交人工确认
 */
function validateStateMachineClosure(errors, warnings, customOperations, fieldList, raw) {
  const stateFields = new Map(); // fieldName -> { field, edges: Map<fromKey, Set<toKey>>, wildcardTo: Set<toKey>, hasWildcardOp: bool }
  const keyOf = (value) => JSON.stringify(value);

  const entryFor = (field) => {
    if (!stateFields.has(field.name)) {
      stateFields.set(field.name, { field, edges: new Map(), wildcardTo: new Set(), hasWildcardOp: false });
    }
    return stateFields.get(field.name);
  };

  // narrowing：一条操作的某字段前置集合（从"全部状态"出发收窄）；返回 null 表示通配
  const narrowFrom = (preconditions, fieldName, enumKeys) => {
    const onField = (preconditions || []).filter((pc) => pc && pc.field === fieldName);
    if (onField.length === 0) return null;
    let set = new Set(enumKeys);
    for (const pc of onField) {
      if (pc.operator === "equals") set = new Set([...set].filter((k) => k === keyOf(pc.value)));
      else if (pc.operator === "in") {
        const allowed = new Set((Array.isArray(pc.value) ? pc.value : []).map(keyOf));
        set = new Set([...set].filter((k) => allowed.has(k)));
      } else if (pc.operator === "notEquals") set = new Set([...set].filter((k) => k !== keyOf(pc.value)));
      else if (pc.operator === "notIn") {
        const banned = new Set((Array.isArray(pc.value) ? pc.value : []).map(keyOf));
        set = new Set([...set].filter((k) => !banned.has(k)));
      }
      // isNull/notNull 不按具体枚举值收窄，保持当前集合
    }
    return set;
  };

  for (const operation of customOperations || []) {
    if (!operation) continue; // stateTransition/command/batch 只要带枚举字段的字面 patch，都构成状态转移边
    for (const patchItem of operation.patch || []) {
      if (!Object.prototype.hasOwnProperty.call(patchItem, "value")) continue; // fromRequest 动态值不参与图分析
      const target = fieldList.find((field) => field.name === patchItem.field);
      if (!target || !Array.isArray(target.enumValues)) continue;
      const entry = entryFor(target);
      const enumKeys = target.enumValues.map(keyOf);
      const fromSet = narrowFrom(operation.preconditions, target.name, enumKeys);
      const toKey = keyOf(patchItem.value);
      if (fromSet === null) {
        entry.hasWildcardOp = true;
        entry.wildcardTo.add(toKey);
        continue;
      }
      if (fromSet.size === 0) {
        fail(errors, `$.customOperations[?].${operation.name}.preconditions`,
          `前置状态组合在字段 ${target.name} 的 enumValues 内为空集，操作永不可触发——请修正前置条件`);
        continue;
      }
      for (const fromKey of fromSet) {
        if (!entry.edges.has(fromKey)) entry.edges.set(fromKey, new Set());
        entry.edges.get(fromKey).add(toKey);
      }
    }
  }

  for (const { field, edges, wildcardTo, hasWildcardOp } of stateFields.values()) {
    const enumKeys = field.enumValues.map(keyOf);
    const initialKey = Object.prototype.hasOwnProperty.call(field, "initialValue") ? keyOf(field.initialValue) : null;
    if (initialKey === null || !enumKeys.includes(initialKey)) continue; // 无初始值（ALTER 场景）不做闭环分析
    // BFS 可达性：通配操作从任何可达状态出发都能命中其目标
    const reachable = new Set([initialKey]);
    const queue = [initialKey];
    while (queue.length > 0) {
      const current = queue.shift();
      const next = new Set([...(edges.get(current) || [])]);
      if (hasWildcardOp) for (const toKey of wildcardTo) next.add(toKey);
    ...
    const unreachable = enumKeys.filter((enumKey) => !reachable.has(enumKey));
    for (const value of unreachable) {
      fail(errors, `$.fields[?].enumValues`,
        `状态机字段 ${field.name} 的枚举值 ${JSON.parse(value)} 不可达：从初始值 ${JSON.stringify(field.initialValue)} 出发没有任何状态转移能到达（补充操作或删除冗余枚举）`);
    }
    // 终态：无出边的状态（通配操作让所有状态都有出边）
    const terminalStates = enumKeys.filter((enumKey) => {
      if (hasWildcardOp) return false;
      return (edges.get(enumKey) || new Set()).size === 0;
    });
    if (terminalStates.length === 0) {
      warnings.push(`状态机字段 ${field.name} 无终态：所有枚举值均有出边。循环流程（如启用/停用）可能是有意设计，请人工确认业务闭环是否符合预期`);
    }
  }
}
...
function validateAlter(errors, rawAlter, location, database, allowedColumns) {
  ...
  const allowed = ["version", "phase", "approvalRef", "impactRef", "rollbackStrategy", "verificationSql", "operations", "indexes"];
  ...
  if (rawAlter.impactRef !== undefined) checkString(errors, rawAlter.impactRef, `${location}.impactRef`, null, 300);
  ...
  const fieldKeys = ["name", "column", "javaType", "dbType", "comment", "requiredOnCreate", "queryMode", "maxLength", "nullable", "defaultValue"];
  const ops = rawAlter.operations.map((op, i) => {
    ...
    if (op.type === "add") {
      ...
      checkString(errors, f.column, `${loc}.field.column`, physicalIdentifierPattern(database));
      if (forbiddenJavaField(f.name)) {
        fail(errors, `${loc}.field.name`, `禁止 isXxx 字段；Java Bean get/set 与序列化映射存在歧义，请改为 ${javaFlagSuggestion(f.name)}`);
      }
      if (forbiddenDatabaseField(f.column)) {
        fail(errors, `${loc}.field.column`, `禁止 is_ 前缀；请先修正需求/设计文档并改为 ${databaseFlagSuggestion(f.column)}`);
      }
      ...
      if (f.nullable === false) fail(errors, `${loc}.field.nullable`, "expand 阶段新增列必须 nullable=true；回填完成后再用独立迁移收紧约束");
      if (Object.prototype.hasOwnProperty.call(f, "defaultValue") && !patchValueKind(f.javaType, f.defaultValue)) {
        fail(errors, `${loc}.field.defaultValue`, "默认值类型与 javaType 不匹配");
      }
      return { type: "add", field: f };
    }
    if (op.type === "drop") {
      checkString(errors, op.column, `${loc}.column`, physicalIdentifierPattern(database));
...
function validateValidationRules(errors, rawRules, location, fields) {
  ...
  return rawRules.map((rule, index) => {
    const loc = `${location}[${index}]`;
    const keys = ["kind", "startField", "endField", "allowEqual", "operations", "message", "source"];
    if (!checkKeys(errors, rule, loc, keys, ["kind", "startField", "endField", "operations", "message", "source"])) return null;
    if (rule.kind !== "chronology") fail(errors, `${loc}.kind`, "当前只支持显式 chronology 规则");
    checkString(errors, rule.startField, `${loc}.startField`, /^[a-z][A-Za-z0-9]*$/);
    checkString(errors, rule.endField, `${loc}.endField`, /^[a-z][A-Za-z0-9]*$/);
    checkString(errors, rule.message, `${loc}.message`, null, 100);
    checkString(errors, rule.source, `${loc}.source`, null, 300);
    if (rule.startField === rule.endField) fail(errors, loc, "开始和结束字段不能相同");
    const start = byName.get(rule.startField);
    const end = byName.get(rule.endField);
    if (!start) fail(errors, `${loc}.startField`, "字段不存在");
    if (!end) fail(errors, `${loc}.endField`, "字段不存在");
    if (start && end && (start.javaType !== end.javaType || !["LocalDate", "LocalDateTime"].includes(start.javaType))) {
      fail(errors, loc, "chronology 两端必须是相同的 LocalDate/LocalDateTime 类型");
    }
    if (rule.allowEqual !== undefined && typeof rule.allowEqual !== "boolean") fail(errors, `${loc}.allowEqual`, "必须是布尔值");
    if (!Array.isArray(rule.operations) || rule.operations.length === 0
      || rule.operations.some((item) => !["create", "update", "page"].includes(item))) {
      fail(errors, `${loc}.operations`, "必须是 create/update/page 的非空数组");
    } else if (new Set(rule.operations).size !== rule.operations.length) {
      fail(errors, `${loc}.operations`, "不能重复");
    }
    for (const operation of rule.operations || []) {
      if (operation === "page" && start && end
        && (start.queryMode === "none" || end.queryMode === "none")) {
        fail(errors, `${loc}.operations`, "page chronology 字段必须都进入 pageRequest");
      }
      if (["create", "update"].includes(operation) && start && end
        && (!start.writable || !end.writable)) {
        fail(errors, `${loc}.operations`, `${operation} chronology 字段必须都可写`);
      }
    }
    return {
      ...rule,
      allowEqual: rule.allowEqual !== false,
    };
  }).filter(Boolean);
}

function validateDatabaseTarget(errors, value, location, production) {
  if (value === undefined) {
    if (production) fail(errors, location, "生产保障契约必须声明数据库目标与变更风险");
    return undefined;
  }
  const allowed = [
    "schema", "location", "estimatedRows", "estimatedBytes", "downtimeBudgetSeconds",
    "onlineDdl", "backupRef", "recoveryOwner",
  ];
  const required = production ? allowed : ["schema", "location"];
  if (!checkKeys(errors, value, location, allowed, required)) return value;
  checkString(errors, value.schema, `${location}.schema`, /^[A-Za-z][A-Za-z0-9_$#]{0,127}$/);
  checkString(errors, value.location, `${location}.location`, null, 200);
  for (const key of ["estimatedRows", "estimatedBytes", "downtimeBudgetSeconds"]) {
    if (value[key] !== undefined && (!Number.isSafeInteger(value[key]) || value[key] < 0)) {
      fail(errors, `${location}.${key}`, "必须是非负安全整数");
    }
  }
  if (value.onlineDdl !== undefined && typeof value.onlineDdl !== "boolean") fail(errors, `${location}.onlineDdl`, "必须是布尔值");
  if (value.backupRef !== undefined) checkString(errors, value.backupRef, `${location}.backupRef`, null, 300);
  if (value.recoveryOwner !== undefined) checkString(errors, value.recoveryOwner, `${location}.recoveryOwner`, null, 100);
  return value;
}

function validateErrorCatalog(errors, value, location, operationNames, production) {
  if (value === undefined) {
    if (production) fail(errors, location, "生产保障契约必须声明业务错误目录");
    return [];
  }
  ...
  if (value.length > 200) fail(errors, location, "最多允许 200 个错误定义");
  const codes = new Set();
  return value.map((entry, index) => {
    const itemLocation = `${location}[${index}]`;
    if (!checkKeys(errors, entry, itemLocation, ["code", "httpStatus", "message", "owner", "retryable", "operations"], ["code", "httpStatus", "message", "owner", "retryable", "operations"])) return null;
    checkString(errors, entry.code, `${itemLocation}.code`, /^[A-Z][A-Z0-9_]{2,63}$/);
    if (codes.has(entry.code)) fail(errors, `${itemLocation}.code`, "错误码重复");
    codes.add(entry.code);
    if (!Number.isInteger(entry.httpStatus) || entry.httpStatus < 400 || entry.httpStatus > 599) fail(errors, `${itemLocation}.httpStatus`, "必须是 400~599 的 HTTP 状态码");
    checkString(errors, entry.message, `${itemLocation}.message`, null, 200);
    checkString(errors, entry.owner, `${itemLocation}.owner`, null, 100);
    if (typeof entry.retryable !== "boolean") fail(errors, `${itemLocation}.retryable`, "必须是布尔值");
    if (!Array.isArray(entry.operations) || entry.operations.length === 0) fail(errors, `${itemLocation}.operations`, "至少关联一个操作");
    else {
      if (new Set(entry.operations).size !== entry.operations.length) fail(errors, `${itemLocation}.operations`, "操作不能重复");
      for (const [operationIndex, operation] of entry.operations.entries()) {
        if (!operationNames.has(operation)) fail(errors, `${itemLocation}.operations[${operationIndex}]`, `引用了不存在的操作 ${operation}`);
      }
    }
    return entry;
  }).filter(Boolean);
...
function validateContract(raw, options = {}) {
  ...
  const errors = [];
  const topKeys = [
    "schemaVersion", "contractKind", "contractId", "profile", "rootPackage", "module", "externalId",
    "environment", "dbCluster", "databaseTarget", "generation",
    "entity", "api", "database", "migration", "output", "fields",
    "indexes", "customOperations", "businessKeys", "relations", "validationRules", "alter", "assurance", "errors", "logicalIds", "integrations",
  ];
  const topRequired = topKeys.filter((key) => !["contractKind", "output", "externalId", "environment", "dbCluster", "databaseTarget", "generation", "indexes", "customOperations", "businessKeys", "relations", "validationRules", "alter", "assurance", "errors", "logicalIds", "integrations"].includes(key));
  ...
  if (raw.contractKind !== undefined && raw.contractKind !== "crud") {
    fail(errors, "$.contractKind", "严格代码生成契约只支持 contractKind=crud");
  }
  ...
  const generation = validateGeneration(errors, raw.generation, "$.generation");
  const assurance = validateAssurance(errors, raw.assurance, "$.assurance", projectRoot);
  const production = Boolean(assurance && assurance.level === "production");
  if (production && !raw.dbCluster) fail(errors, "$.dbCluster", "生产保障契约必须声明数据库集群");
  const databaseTarget = validateDatabaseTarget(errors, raw.databaseTarget, "$.databaseTarget", production);
  ...
  if (object(raw.entity) && ["oracle", "mysql"].includes(raw.database)) {
    checkString(errors, raw.entity.table, "$.entity.table", physicalIdentifierPattern(raw.database));
  }
  ...
  const fieldKeys = [
    "name", "column", "javaType", "dbType", "comment", "requiredOnCreate", "requiredOnQuery",
    "writable", "queryMode", "detail", "list", "maxLength", "externalId", "semanticId",
    "constraints", "constraintSource", "queryConstraints", "queryConstraintSource", "contextSource",
    "definition", "enumValues", "initialValue", "classification", "masking", "logPolicy",
    "encryptionRequired", "retentionDays", "dataOwner", "sourceOfTruth", "nullable", "defaultValue",
  ];
  const names = new Set();
  ...
  if (!Array.isArray(raw.fields) || raw.fields.length === 0) {
    ...
    raw.fields.forEach((field, index) => {
      ...
      checkString(errors, field.column, `${location}.column`, physicalIdentifierPattern(raw.database));
      if (forbiddenJavaField(field.name)) {
        fail(errors, `${location}.name`, `禁止 isXxx 字段；Java Bean get/set 与序列化映射存在歧义，请改为 ${javaFlagSuggestion(field.name)}`);
      }
      if (forbiddenDatabaseField(field.column)) {
        fail(errors, `${location}.column`, `禁止 is_ 前缀；请先修正需求/设计文档并改为 ${databaseFlagSuggestion(field.column)}`);
      }
      ...
      if (governanceColumns(raw.database).has(field.column)) {
      ...
      if (production && (field.detail !== false || field.list !== false) && field.semanticId === undefined) {
        fail(errors, `${location}.semanticId`, "生产保障下对外响应字段必须声明稳定语义 ID");
      }
      ...
      for (const key of ["requiredOnCreate", "requiredOnQuery", "writable", "detail", "list"]) {
        if (field[key] !== undefined && typeof field[key] !== "boolean") fail(errors, `${location}.${key}`, "必须是布尔值");
      }
      if (field.nullable !== undefined && typeof field.nullable !== "boolean") fail(errors, `${location}.nullable`, "必须是布尔值");
      if (field.requiredOnCreate === true && field.nullable === true) {
        fail(errors, location, "requiredOnCreate=true 与 nullable=true 语义冲突；API 必填字段不得生成可空物理列");
      }
      if (Object.prototype.hasOwnProperty.call(field, "defaultValue")
        && !patchValueKind(field.javaType, field.defaultValue)) {
        fail(errors, `${location}.defaultValue`, "默认值类型与 javaType 不匹配");
      }
      if (Object.prototype.hasOwnProperty.call(field, "defaultValue")
        && Object.prototype.hasOwnProperty.call(field, "initialValue")) {
        fail(errors, location, "defaultValue 与 initialValue 只能声明一个");
      }
      ...
      if (field.requiredOnQuery === true && (!field.queryMode || field.queryMode === "none")) {
        fail(errors, location, "requiredOnQuery=true 时 queryMode 必须为 eq/like");
      }
      if (field.contextSource !== undefined && !["client", "server"].includes(field.contextSource)) {
        fail(errors, `${location}.contextSource`, "只允许 client/server");
      }
      if (field.contextSource === "server"
        && (field.writable !== false || (field.queryMode !== undefined && field.queryMode !== "none")
          || field.requiredOnCreate === true || field.requiredOnQuery === true)) {
        fail(errors, location, "server 上下文字段不得进入查询/新增/修改请求；必须 writable=false、queryMode=none");
      }
      ...
      validateConstraints(errors, field.constraints, `${location}.constraints`, field.javaType);
      validateConstraints(errors, field.queryConstraints, `${location}.queryConstraints`, field.javaType);
      if (field.constraints !== undefined && field.constraintSource === undefined) {
        fail(errors, `${location}.constraintSource`, "声明 constraints 时必须提供可追溯来源");
      }
      if (field.queryConstraints !== undefined && field.queryConstraintSource === undefined) {
        fail(errors, `${location}.queryConstraintSource`, "声明 queryConstraints 时必须提供查询契约来源");
      }
      for (const key of ["constraintSource", "queryConstraintSource"]) {
        if (field[key] !== undefined) checkString(errors, field[key], `${location}.${key}`, null, 300);
      }
      const storage = dbStorageConstraints(field);
      const write = effectiveWriteConstraints(field);
      if (storage.maxLength && write.maxLength && write.maxLength > storage.maxLength) {
        fail(errors, `${location}.constraints.maxLength`, `写入长度 ${write.maxLength} 不能超过数据库容量 ${storage.maxLength}`);
      }
      if (storage.totalDigits && write.totalDigits && write.totalDigits > storage.totalDigits) {
        fail(errors, `${location}.constraints.totalDigits`, `写入精度 ${write.totalDigits} 不能超过数据库精度 ${storage.totalDigits}`);
      }
      fieldList.push(field);
    });
  }

  const allowedColumns = new Set([...governanceColumns(raw.database), ...columns]);
  ...
  const businessKeys = validateBusinessKeys(errors, raw.businessKeys, "$.businessKeys", fieldList, production);
  const operationNames = new Set([...STANDARD_OPERATIONS, ...(raw.api && raw.api.permissions && raw.api.permissions.export ? ["export"] : []), ...customOperations.map((operation) => operation.name)]);
  const errorCatalog = validateErrorCatalog(errors, raw.errors, "$.errors", operationNames, production);
  const logicalIdResult = validateLogicalIds(raw.logicalIds, fieldList);
  for (const item of logicalIdResult.issues) fail(errors, item.path, item.message);
  const integrationResult = validateIntegrations(raw.integrations, {
    logicalIds: logicalIdResult.values,
    operations: [...operationNames],
    errors: errorCatalog,
    fields: fieldList,
  });
  for (const item of integrationResult.issues) fail(errors, item.path, item.message);
  ...
  validateStateMachineClosure(errors, closureWarnings, customOperations, fieldList, raw);
  const relations = validateRelations(errors, raw.relations, "$.relations", raw.contractId, raw.database);
  const validationRules = validateValidationRules(errors, raw.validationRules, "$.validationRules", fieldList);
  ...
  const contract = {
    ...
    ...(businessKeys.length > 0 ? { businessKeys } : {}),
    ...(relations.length > 0 ? { relations } : {}),
    ...(validationRules.length > 0 ? { validationRules } : {}),
    ...(alter ? { alter } : {}),
    ...(assurance ? { assurance } : {}),
    ...(databaseTarget ? { databaseTarget } : {}),
    ...(errorCatalog.length > 0 ? { errors: errorCatalog } : {}),
    ...(logicalIdResult.values.length > 0 ? { logicalIds: logicalIdResult.values } : {}),
    ...(integrationResult.values.length > 0 ? { integrations: integrationResult.values } : {}),
    ...(generation ? { generation } : {}),
  };
  return {
    ok: true,
    errors: [],
    warnings: [...(profileInfo.profileWarnings || []), ...closureWarnings],
    contract,
...
function validationAnnotations(field, validationMode) {
  const annotations = [];
  const required = validationMode === "create"
    ? field.requiredOnCreate
    : validationMode === "query" && field.requiredOnQuery;
  if (required) {
    annotations.push(field.javaType === "String"
      ? `@NotBlank(message = "${field.comment}不能为空")`
      : `@NotNull(message = "${field.comment}不能为空")`);
  }
  const constraints = validationMode === "query"
    ? effectiveQueryConstraints(field)
    : ["create", "update"].includes(validationMode)
      ? effectiveWriteConstraints(field)
      : {};
  if (constraints.minLength !== undefined || constraints.maxLength !== undefined) {
    const args = [];
    if (constraints.minLength !== undefined) args.push(`min = ${constraints.minLength}`);
    if (constraints.maxLength !== undefined) args.push(`max = ${constraints.maxLength}`);
    args.push(`message = "${field.comment}长度不符合要求"`);
    annotations.push(`@Size(${args.join(", ")})`);
  }
  if (constraints.pattern !== undefined) {
    annotations.push(`@Pattern(regexp = ${javaString(constraints.pattern)}, message = "${field.comment}格式不正确")`);
  }
  if (constraints.minimum !== undefined) {
    annotations.push(`@DecimalMin(value = "${constraints.minimum}", inclusive = ${constraints.minExclusive !== true}, message = "${field.comment}不能小于${constraints.minimum}")`);
  }
  if (constraints.maximum !== undefined) {
    annotations.push(`@DecimalMax(value = "${constraints.maximum}", inclusive = ${constraints.maxExclusive !== true}, message = "${field.comment}不能大于${constraints.maximum}")`);
  }
  if (constraints.totalDigits !== undefined) {
    annotations.push(`@Digits(integer = ${constraints.totalDigits - Number(constraints.fractionDigits || 0)}, fraction = ${Number(constraints.fractionDigits || 0)}, message = "${field.comment}精度不符合要求")`);
  }
  return annotations;
}

function fieldView(field, validationMode) {
  const annotations = validationAnnotations(field, validationMode);
  ...
  return {
    ...
    effectiveConstraints: validationMode === "query"
      ? effectiveQueryConstraints(field)
      : effectiveWriteConstraints(field),
...
function customOperationContext(op, contract, idPolicy) {
  idPolicy = idPolicy || resolveProfilePolicies().idPolicy;
  ...
  const serviceParams = [];
  if (hasId) {
    controllerParams.push(`@Parameter(description = "主键ID")
                    @Size(max = ${idPolicy.maxLength}, message = "主键长度不能超过${idPolicy.maxLength}")
                    @PathVariable("id")
                    String id`);
    serviceArgs.push("id");
  ...
  const batchPolicy = op.batchPolicy ? {
    minItems: op.batchPolicy.minItems,
    minItemsMessage: op.batchPolicy.messages.minItems,
    sameFieldsMessage: op.batchPolicy.messages.sameFields,
    distinctMessage: op.batchPolicy.messages.distinct,
    source: op.batchPolicy.source,
    sameFields: op.batchPolicy.sameFields.map((field) => ({ field, Field: upperFirst(field) })),
    distinct: op.batchPolicy.distinct.map((item) => ({
      field: item.field,
      Field: upperFirst(item.field),
      variable: `${item.field}DistinctValues`,
      count: item.count,
    })),
  } : null;
  return {
    ...
    requestUsesSize: hasBodyId || hasBodyIds || requestFields.some((field) => field.maxLength),
    idMaxLength: idPolicy.maxLength,
    controllerParams,
    ...
    batchPolicy,
    batchPolicyPresent: Boolean(batchPolicy),
    batchPolicySameFieldsPresent: Boolean(batchPolicy && batchPolicy.sameFields.length > 0),
    externalId: op.externalId,
  };
}

function businessKeyContext(key) {
  const fields = [...(key.scopeFields || []), ...key.fields];
  return {
    name: key.name,
    Name: upperFirst(key.name),
    fields: fields.map((field) => ({ field, Field: upperFirst(field) })),
    message: key.message,
    source: key.source,
    databaseConstraintRef: key.databaseConstraintRef,
    forCreate: key.operations.includes("create"),
    forUpdate: key.operations.includes("update"),
  };
}
...
function crossFieldRuleContext(rule, index) {
  const operator = rule.allowEqual === false ? "< 0" : "<= 0";
  return {
    methodName: `isChronology${index + 1}Valid`,
...
function crossFieldRulesFor(contract, operation) {
  return (contract.validationRules || [])
    .filter((rule) => rule.operations.includes(operation))
    .map(crossFieldRuleContext);
}

function operationContext(deliveryProfile, name) {
  const operation = deliveryProfile.transport.operations[name];
  const annotation = {
  ...
  return {
    ...
    path: operation.path,
    annotation,
    requestAnnotation: ["POST", "PUT", "PATCH"].includes(operation.method)
      ? "@RequestBody @Validated"
      : "@Parameter(hidden = true) @Validated",
  };
}

function buildContext(contract, profile, deliveryProfile) {
  const activeDeliveryProfile = deliveryProfile || readJson(path.join(
    PACKAGE_FILES, ".wl-skills-bd", "contracts", DELIVERY_PROFILE_NAME,
  ));
  const governance = resolveGovernance(profile);
  const { idPolicy } = resolveProfilePolicies(profile);
  const softDelete = {
    ...governance.softDelete,
    column: physicalIdentifier(contract.database, governance.softDelete.column),
  };
  ...
  const queryFields = contract.fields.filter((field) => field.queryMode !== "none").map((field) => fieldView(field, "query"));
  ...
  const validationImports = (fields, mode) => {
    ...
    const annotations = fields.flatMap((field) => validationAnnotations(field, mode));
    const types = {
    ...
    for (const [annotation, importName] of Object.entries(types)) {
      if (annotations.some((value) => value.startsWith(`@${annotation}`))) result.push(importName);
    }
    return [...new Set(result)].sort();
  };
  const customOps = (contract.customOperations || []).map((op) => customOperationContext(op, contract, idPolicy));
  const businessKeys = (contract.businessKeys || []).map(businessKeyContext);
  const normalizationFields = [...new Set((contract.businessKeys || [])
    .filter((key) => key.normalization === "trim")
    .flatMap((key) => [...(key.scopeFields || []), ...key.fields]))]
    .map((field) => ({ field, Field: upperFirst(field) }));
  ...
  const parameterId = `@Parameter(description = "主键ID")
                    @Size(max = ${idPolicy.maxLength}, message = "主键长度不能超过${idPolicy.maxLength}")
                    @PathVariable("id")
                    String id`;
  const pagination = activeDeliveryProfile.transport.pagination || {
    defaultCurrent: 1,
    defaultSize: 10,
    maxSize: 200,
  };
  const createCrossFieldRules = crossFieldRulesFor(contract, "create");
  const updateCrossFieldRules = crossFieldRulesFor(contract, "update");
  const pageCrossFieldRules = crossFieldRulesFor(contract, "page");
  const standardOperations = Object.fromEntries(STANDARD_OPERATIONS.map((name) => [
    name,
    operationContext(activeDeliveryProfile, name),
  ]));
  return {
    ...
    idMaxLength: idPolicy.maxLength,
    idFormat: idPolicy.format,
    idColumn: physicalIdentifier(contract.database, "ID"),
    companyIdColumn: physicalIdentifier(contract.database, "COMPANY_ID"),
    revisionColumn: physicalIdentifier(contract.database, "REVISION"),
    createUserNoColumn: physicalIdentifier(contract.database, "CREATE_USER_NO"),
    updateUserNoColumn: physicalIdentifier(contract.database, "UPDATE_USER_NO"),
    createDateTimeColumn: physicalIdentifier(contract.database, "CREATE_DATE_TIME"),
    updateDateTimeColumn: physicalIdentifier(contract.database, "UPDATE_DATE_TIME"),
    successCode: profile.response.successCode,
    pageDefaultCurrent: pagination.defaultCurrent,
    pageDefaultSize: pagination.defaultSize,
    pageMaxSize: pagination.maxSize,
    pageHttpAnnotation: standardOperations.page.annotation,
    pageOperationPath: standardOperations.page.path,
    pageRequestAnnotation: standardOperations.page.requestAnnotation,
    detailHttpAnnotation: standardOperations.detail.annotation,
    detailOperationPath: standardOperations.detail.path,
    createHttpAnnotation: standardOperations.create.annotation,
    createOperationPath: standardOperations.create.path,
    createRequestAnnotation: standardOperations.create.requestAnnotation,
    updateHttpAnnotation: standardOperations.update.annotation,
    updateOperationPath: standardOperations.update.path,
    updateRequestAnnotation: standardOperations.update.requestAnnotation,
    removeHttpAnnotation: standardOperations.remove.annotation,
    removeOperationPath: standardOperations.remove.path,
    createCrossFieldRules,
    updateCrossFieldRules,
    pageCrossFieldRules,
    createHasCrossFieldRules: createCrossFieldRules.length > 0,
    updateHasCrossFieldRules: updateCrossFieldRules.length > 0,
    pageHasCrossFieldRules: pageCrossFieldRules.length > 0,
    softDeleteColumn: softDelete.column,
    softDeleteJavaField: softDelete.javaField,
    softDeleteJavaFieldUpper: softDelete.javaField.charAt(0).toUpperCase() + softDelete.javaField.slice(1),
    ...
    pageDtoImports: validationImports(contract.fields.filter((field) => field.queryMode !== "none"), "query")
      .filter((name) => !["javax.validation.constraints.NotNull"].includes(name)),
    ...
    usesPatchMapping: customOps.some((operation) => operation.httpAnnotation === "PatchMapping")
      || Object.values(standardOperations).some((operation) => operation.annotation === "PatchMapping"),
    customRequestDtos: customOps.filter((operation) => operation.requestDtoPresent),
    businessKeys,
    createBusinessKeys: businessKeys.filter((key) => key.forCreate),
    updateBusinessKeys: businessKeys.filter((key) => key.forUpdate),
    createBusinessKeysPresent: businessKeys.some((key) => key.forCreate),
    updateBusinessKeysPresent: businessKeys.some((key) => key.forUpdate),
    normalizationFields,
    normalizationFieldsPresent: normalizationFields.length > 0,
