...
function validateIndexes(errors, rawIndexes, location, database) {
  ...
  return rawIndexes.map((entry, index) => {
    ...
    if (!Array.isArray(entry.columns) || entry.columns.length === 0) {
      ...
      entry.columns.forEach((col, ci) => checkString(errors, col, `${loc}.columns[${ci}]`, /^[A-Z][A-Z0-9_]*$/));
    }
...
function validateCustomField(errors, raw, location, contractFieldNames) {
  if (!checkKeys(errors, raw, location, ["name", "javaType", "comment", "required", "maxLength"], ["name", "javaType", "comment"])) return null;
...
function validateCustomOperations(errors, rawOps, location, contractFields, contractFieldNames, permissionPrefix) {
  ...
  return rawOps.map((op, index) => {
    ...
    if (op.requestFields !== undefined) {
      ...
      else requestFields = op.requestFields.map((rf, ri) => validateCustomField(errors, rf, `${loc}.requestFields[${ri}]`, contractFieldNames));
    }
    ...
    if (op.patch !== undefined) {
      if (!Array.isArray(op.patch)) fail(errors, `${loc}.patch`, "必须是数组");
        ...
        if (!checkKeys(errors, pa, pLoc, ["field", "value"], ["field", "value"])) return;
        const target = fieldByName.get(pa.field);
        if (!target) fail(errors, `${pLoc}.field`, `引用了不存在的契约字段 ${pa.field}`);
        else if (!patchValueKind(target.javaType, pa.value)) fail(errors, `${pLoc}.value`, `值类型与字段 ${pa.field} (${target.javaType}) 不匹配`);
      });
    }
...
function validateAlter(errors, rawAlter, location, database) {
  if (rawAlter === undefined) return null;
  const allowed = ["version", "rollbackStrategy", "verificationSql", "operations"];
  if (!checkKeys(errors, rawAlter, location, allowed, ["version", "rollbackStrategy", "verificationSql", "operations"])) return null;
  ...
  if (!Array.isArray(rawAlter.verificationSql) || rawAlter.verificationSql.length === 0) {
    fail(errors, `${location}.verificationSql`, "必须至少包含一条只读验证 SQL");
  } else {
    rawAlter.verificationSql.forEach((sql, i) => {
      if (!checkString(errors, sql, `${location}.verificationSql[${i}]`)) return;
      if (!/^SELECT\b/i.test(sql.trim()) || /;/.test(sql)) fail(errors, `${location}.verificationSql[${i}]`, "只能填写不带分号的 SELECT 验证语句");
    });
  }
  ...
  const ops = rawAlter.operations.map((op, i) => {
    ...
    if (!checkKeys(errors, op, loc, ["type", "field", "column", "dbType", "comment"], ["type"])) return null;
    ...
    return { type: "modify", column: op.column, dbType: op.dbType, comment: op.comment };
  }).filter(Boolean);
  return { ...rawAlter, operations: ops };
}
...
function validateContract(raw, options = {}) {
  ...
  const topKeys = [
    ...
    "indexes", "customOperations", "relations", "alter",
  ];
  const topRequired = topKeys.filter((key) => !["output", "externalId", "environment", "dbCluster", "indexes", "customOperations", "relations", "alter"].includes(key));
  ...
  if (checkKeys(errors, raw.migration, "$.migration", ["version", "rollbackStrategy", "verificationSql"], ["version", "rollbackStrategy", "verificationSql"])) {
    ...
    if (!Array.isArray(raw.migration.verificationSql) || raw.migration.verificationSql.length === 0) {
      fail(errors, "$.migration.verificationSql", "必须至少包含一条只读验证 SQL");
    } else {
      raw.migration.verificationSql.forEach((sql, index) => {
        if (!checkString(errors, sql, `$.migration.verificationSql[${index}]`)) return;
        if (!/^SELECT\b/i.test(sql.trim()) || /;/.test(sql)) {
          fail(errors, `$.migration.verificationSql[${index}]`, "只能填写不带分号的 SELECT 验证语句");
        }
      });
    }
  }
  ...
  const fieldKeys = [
    ...
    "writable", "queryMode", "detail", "list", "maxLength", "externalId",
  ];
  const names = new Set();
  ...
  if (!Array.isArray(raw.fields) || raw.fields.length === 0) {
    ...
    raw.fields.forEach((field, index) => {
      ...
      if (!checkKeys(errors, field, location, fieldKeys, ["name", "column", "javaType", "dbType", "comment"])) return;
  ...
  const indexes = validateIndexes(errors, raw.indexes, "$.indexes", raw.database);
  ...
  const alter = validateAlter(errors, raw.alter, "$.alter", raw.database);
...
function customFieldView(field) {
  return {
    ...
    fieldType: field.javaType,
    fieldComment: field.comment.replace(/[\r\n]/g, " "),
    validationAnnotations: field.required
...
function preconditionJava(pc, entityName, javaType) {
  ...
  if (pc.operator === "equals") return `ServiceAssert.isTrue(java.util.Objects.equals(${actual}, ${v}), "${pc.message}")`;
  if (pc.operator === "notEquals") return `ServiceAssert.isTrue(!java.util.Objects.equals(${actual}, ${v}), "${pc.message}")`;
  if (pc.operator === "in") {
    const set = `java.util.Arrays.asList(${pc.value.map((x) => literalValue(javaType, x)).join(", ")})`;
    return `ServiceAssert.isTrue(${set}.contains(${actual}), "${pc.message}")`;
  }
...
function customOperationContext(op, contract) {
  ...
  const preconditions = (op.preconditions || []).map((pc) => {
    ...
    return { java: preconditionJava(pc, entityVar, target.javaType), message: pc.message };
  });
  const patchLines = (op.patch || []).map((pa) => {
    ...
    return { field: pa.field, Field: upperFirst(pa.field), value: literalValue(javaType, pa.value) };
  });
  ...
  if (hasBodyIds) {
    controllerParams.push('@RequestBody java.util.List<String> ids');
    serviceArgs.push("ids");
    serviceParams.push("java.util.List<String> ids");
  }
  if (hasBodyId) {
    controllerParams.push('@RequestBody String id');
    serviceArgs.push("id");
    serviceParams.push("String id");
  }
  for (const rf of requestFields) {
    const paramAnnotation = rf.required
      ? `@RequestParam("${rf.name}")`
      : `@RequestParam(value = "${rf.name}", required = false)`;
    controllerParams.push(`${paramAnnotation} ${rf.fieldType} ${rf.field}`);
    serviceArgs.push(rf.field);
    serviceParams.push(`${rf.fieldType} ${rf.field}`);
  }
...
function buildContext(contract, profile) {
  ...
  const fieldsForImport = [...contract.fields, ...(contract.customOperations || []).flatMap((op) => op.requestFields || [])];
  return {
    ...
    usesRequestParam: customOps.some((op) => op.requestFieldsPresent),
    hasExport,
    exportPresent: hasExport,
    alter: contract.alter || null,
    isAlter: Boolean(contract.alter),
    importsForRequestFields: importsFor(fieldsForImport),
  };
}
