...
function resolveProfile(profileId, projectRoot) {
  ...
  if (fs.existsSync(localOverrideFile)) {
    ...
    const allowedKeys = new Set(["schemaVersion", "profileId", "softDelete", "auditTime"]);
    ...
    if (!localOverride.softDelete && !localOverride.auditTime) {
      throw new Error(`${localOverrideFile}: profile.local.json 至少覆盖 softDelete 或 auditTime`);
    }
...
function validateContract(raw, options = {}) {
  ...
  const topKeys = [
    ...
    "indexes", "customOperations", "relations", "validationRules", "alter", "assurance",
  ];
  const topRequired = topKeys.filter((key) => !["output", "externalId", "environment", "dbCluster", "indexes", "customOperations", "relations", "validationRules", "alter", "assurance"].includes(key));
