...
const SECURITY_CONFIG_RULES = Object.freeze([
  {
    key: "spring.main.allow-bean-definition-overriding",
    unsafe: (value) => /^true$/iu.test(value),
    message: "禁止开启 Bean 定义覆盖；重复 Bean 应在装配阶段显式失败",
  },
  {
    key: "management.endpoints.web.exposure.include",
    unsafe: (value) => value.split(",").map((item) => item.trim()).includes("*"),
    message: "Actuator 不得暴露全部端点；使用 health,info,prometheus 等最小白名单",
  },
  {
    key: "management.endpoint.env.show-values",
    unsafe: (value) => /^always$/iu.test(value),
    message: "env 端点不得无条件展示配置值",
  },
  {
    key: "management.endpoint.configprops.show-values",
    unsafe: (value) => /^always$/iu.test(value),
    message: "configprops 端点不得无条件展示配置值",
  },
  {
    key: "server.error.include-stacktrace",
    unsafe: (value) => /^always$/iu.test(value),
    message: "响应不得无条件包含异常堆栈",
  },
  {
    key: "server.error.include-binding-errors",
    unsafe: (value) => /^always$/iu.test(value),
    message: "响应不得无条件包含绑定错误内部细节",
  },
]);

function scanSecurityConfig(files) {
  const issues = [];
  for (const file of files) {
    const content = readFileSyncSafe(file.abs);
    if (!content) continue;
    for (const entry of parseYamlKeyValue(content)) {
      const rule = SECURITY_CONFIG_RULES.find((candidate) => candidate.key === entry.path.toLowerCase());
      if (!rule || !rule.unsafe(String(entry.value).trim())) continue;
      issues.push({ file: file.rel, line: entry.line, key: entry.path, value: entry.value, message: rule.message });
    }
  }
  return issues;
}

function runConfigDoctorSingle(root, options = {}) {
  const checks = [];
  ...
  if (!bootstrap) {
    ...
    } else if (bootstrap.profileDeclared && /\$\{(?:PROFILES_ACTIVE|SPRING_PROFILES_ACTIVE)\}/.test(bootstrap.profileRaw || "")) {
      add("config-profile", true, `profile 由运行环境显式注入（${bootstrap.profileRaw}）`);
    } else {
    ...
    if (bootstrap.nacosAddr && bootstrap.namespace && bootstrap.group) {
      ...
      const datasourceIds = bootstrap.datasourceDataIds || [];
      const datasourcePatternOk = datasourceIds.length === 1
        && datasourceIds[0].includes("${DB_CLUSTER")
        && datasourceIds[0].includes("${spring.profiles.active}");
      add(
        "config-datasource-dataid",
        datasourcePatternOk,
        datasourceIds.length ? `datasource dataId=${datasourceIds.join(", ")}` : "未声明 datasource dataId",
        "使用 datasource-${DATASOURCE:mysql}-${DB_CLUSTER:<cx|non_cx|pt>}-${spring.profiles.active}.yml，避免跨业务库误连",
      );
    } else {
  ...
  const securityIssues = scanSecurityConfig(configFiles);
  if (securityIssues.length === 0) {
    add("config-security-posture", true, "Bean 覆盖、Actuator 暴露与错误详情配置符合最小暴露原则");
  } else {
    const details = securityIssues.slice(0, 5).map((item) => `${item.file}:${item.line} ${item.key}`).join("; ");
    add(
      "config-security-posture",
      false,
      `${securityIssues.length} 处高风险配置：${details}${securityIssues.length > 5 ? " ..." : ""}`,
      securityIssues.map((item) => `${item.key}: ${item.message}`).join("；"),
    );
  }

  // L1 config-placeholder：敏感字段占位符合规（与 config-secret 互补）
  if (bootstrap) {
    // bootstrap.yml 的 password 默认值检查（如 ${VAR:DO_NOT_COMMIT} 也是反例）
  ...
  const matrixResult = readMatrix(options.workspaceRoot || root);
  if (!matrixResult.ok) {
    ...
    if (cfg) {
      ...
      if (bootstrap) {
        const datasourceId = (bootstrap.datasourceDataIds || [])[0] || "";
        const clusterDefault = (datasourceId.match(/\$\{DB_CLUSTER:([^}]+)\}/) || [])[1];
        const consistent = datasourceId.includes("${DB_CLUSTER")
          && (!clusterDefault || clusterDefault === cfg.dbCluster);
        add(
          "env-dbcluster-binding",
          consistent,
          consistent
            ? `bootstrap DB_CLUSTER${clusterDefault ? ` 默认=${clusterDefault}` : " 无默认"}，matrix=${cfg.dbCluster}`
            : `bootstrap datasource dataId 与 matrix dbCluster=${cfg.dbCluster} 不一致`,
          "统一 bootstrap datasource dataId 的 DB_CLUSTER 与 env-matrix datasource.cluster",
        );
      }
      ...
      if (manifests.length === 0) {
        ...
        for (const m of manifests) {
          ...
          const dbCluster = fields.DB_CLUSTER;
          ...
          if (dbCluster && dbCluster !== cfg.dbCluster) {
            k8sOk = false;
            k8sDetails.push(`${m.name}: DB_CLUSTER=${dbCluster} 与 env-matrix(${cfg.dbCluster}) 不一致`);
          }
          if (/kind:\s*Deployment/i.test(m.content)) {
            const deploymentChecks = [
              [!/:latest["'\s]*$/im.test(m.content), "禁止 latest 镜像"],
              [/path:\s*\/actuator\/health\/readiness\b/.test(m.content), "缺独立 readinessProbe"],
              [/path:\s*\/actuator\/health\/liveness\b/.test(m.content), "缺独立 livenessProbe"],
              [/startupProbe\s*:/.test(m.content), "缺 startupProbe"],
              [/automountServiceAccountToken:\s*false/.test(m.content), "未关闭 ServiceAccount Token 自动挂载"],
              [/runAsNonRoot:\s*true/.test(m.content), "未启用 runAsNonRoot"],
              [/allowPrivilegeEscalation:\s*false/.test(m.content), "未禁止提权"],
              [/readOnlyRootFilesystem:\s*true/.test(m.content), "根文件系统未只读"],
              [/terminationGracePeriodSeconds:\s*[3-9]\d|terminationGracePeriodSeconds:\s*\d{3,}/.test(m.content), "优雅终止时间少于 30 秒"],
            ];
            for (const [passed, message] of deploymentChecks) {
              if (!passed) {
                k8sOk = false;
                k8sDetails.push(`${m.name}: ${message}`);
              }
            }
          }
        }
      ...
      if (!portInfo) {
        ...
        const rangeCheck = checkPortRange(matrix.module, portInfo.port, cfg.port);
        add("env-port", rangeCheck.ok, `${rangeCheck.detail}（${portInfo.file}: ${portInfo.raw}）`, rangeCheck.ok ? "" : "先核对现网端口，再同步 env-matrix 或 server.port；禁止盲改可用配置");
      }
      ...
      // L8 protected-write-guard（保留历史检查 id，避免现有报告消费者破坏）
      const env = normalizeEnvironment(process.env.WL_PROJECT_ENV || bootstrapProfile);
      if (process.env.WL_ALLOW_PRODUCTION_WRITES === "true") {
        add("env-production-guard", false, "已设置废弃的 WL_ALLOW_PRODUCTION_WRITES；该变量不会再放行写入", "移除该变量；需要时只对评审过的单次命令显式授权");
      } else if (PROTECTED_ENVS.has(env)) {
        add("env-production-guard", true, `受保护环境 ${env}（工程 apply 已阻断）`);
      } else {
        add("env-production-guard", true, `非受保护环境（${env || "unknown"}）`);
      }
...
function runConfigDoctor(rootInput, options = {}) {
  ...
  if (options.workspace !== false) {
    ...
    if (workspace.enabled) {
      const modules = workspace.modules.map((module) => ({
        id: module.id,
        rel: module.rel,
        result: runConfigDoctorSingle(module.root, { ...options, workspace: false, workspaceRoot: root }),
      }));
      ...
      return {
        ...
        workspace: true,
        checks,
        modules: modules.map((module) => ({ id: module.id, root: module.rel, ...module.result.summary })),
        summary: {
          total: checks.length,
          ok: checks.filter((item) => item.ok).length,
          error: checks.filter((item) => !item.ok && item.severity === "error").length,
          warn: checks.filter((item) => !item.ok && item.severity === "warn").length,
        },
      };
    }
  }
  return runConfigDoctorSingle(root, options);
}
...
module.exports = { runConfigDoctor, runConfigDoctorSingle, probeAllSync, scanSecurityConfig };
