...
function buildTestScenarios(opRaw, contract, profile) {
  const op = customOperationContext(opRaw, contract, resolveProfilePolicies(profile).idPolicy);
  if (op.kind === "batch") {
...
function buildCustomTestsSection(contract, profile) {
  ...
  for (const raw of operations) {
    for (const scenario of buildTestScenarios(raw, contract, profile)) {
      lines.push("");
      lines.push("    @Test");
...
function generateServiceTest(contractFile, options = {}) {
  ...
  const context = buildContext(loaded.contract, loaded.profile, loaded.deliveryProfile);
  const customTestsSection = buildCustomTestsSection(loaded.contract, loaded.profile);
  ...
  return {
    ...
    content,
    scenarioCount: countScenarios(loaded.contract, loaded.profile),
  };
}

function countScenarios(contract, profile) {
  return 1 + (contract.customOperations || [])
    .reduce((count, operation) => count + buildTestScenarios(operation, contract, profile).length, 0);
}
