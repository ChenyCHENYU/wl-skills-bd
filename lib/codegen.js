...
function renderOracleMigration(contract, profile, sourceTable) {
  ...
  const physicalEntries = [
    { key: "id", line: "    ID VARCHAR2(64 CHAR) NOT NULL" },
    ...contract.fields.map((field) => ({ key: String(field.column).toLowerCase(), line: `    ${columnDefinition(field)}` })),
    { key: "company_id", line: "    COMPANY_ID VARCHAR2(64 CHAR) NOT NULL" },
    { key: "is_delete", line: `    ${sd.column} ${sd.oracleType} DEFAULT ${sd.activeValue} NOT NULL` },
    { key: "revision", line: "    REVISION NUMBER(10) DEFAULT 0 NOT NULL" },
    { key: "create_user_no", line: "    CREATE_USER_NO VARCHAR2(64 CHAR)" },
    { key: "update_user_no", line: "    UPDATE_USER_NO VARCHAR2(64 CHAR)" },
    { key: "create_date_time", line: `    CREATE_DATE_TIME ${at.oracleType}` },
    { key: "update_date_time", line: `    UPDATE_DATE_TIME ${at.oracleType}` },
  ];
  const lines = [
...
function renderMysqlMigration(contract, profile, sourceTable) {
  ...
  const physicalEntries = [
    { key: "id", line: "    id VARCHAR(64) NOT NULL COMMENT '主键ID'" },
    ...
    { key: "company_id", line: "    company_id VARCHAR(64) NOT NULL COMMENT '公司/租户ID'" },
    { key: "is_delete", line: `    ${sd.column} ${sd.mysqlType} NOT NULL DEFAULT ${sd.activeValue} COMMENT '${softDeleteComment(sd.activeValue, sd.deletedValue)}'` },
    { key: "revision", line: "    revision INT NOT NULL DEFAULT 0 COMMENT '乐观锁版本号'" },
    { key: "create_user_no", line: "    create_user_no VARCHAR(64) NULL COMMENT '创建人工号'" },
    { key: "update_user_no", line: "    update_user_no VARCHAR(64) NULL COMMENT '更新人工号'" },
    { key: "create_date_time", line: `    create_date_time ${at.mysqlType} NULL COMMENT '创建时间'` },
    { key: "update_date_time", line: `    update_date_time ${at.mysqlType} NULL COMMENT '更新时间'` },
  ];
  const lines = [
    ...orderedPhysicalLines(contract, sourceTable, physicalEntries),
    "    PRIMARY KEY (id)",
