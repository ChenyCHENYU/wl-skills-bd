...
function buildSourceIndex(projectRoot, options = {}) {
  const roots = contractRoots(projectRoot, options);
  const files = walkFiles(projectRoot, roots, (file) => file.toLowerCase().endsWith(".json"));
  const snapshot = snapshotFiles(projectRoot, files);
  const key = hash(JSON.stringify({ roots, contractsRel: options.contractsRel || null, schemaVersion: 1 }));
  const memoryKey = `${path.resolve(projectRoot)}\u0000${key}`;
  const useCache = cacheEnabled(projectRoot, options);
  if (useCache) {
    const memory = memoryCache.get(memoryKey);
    if (memory && memory.snapshot === snapshot) return hydrateIndex(projectRoot, memory.index, "memory");
    const persistent = loadPersistentCache(projectRoot).entries[key];
    if (persistent && persistent.snapshot === snapshot && persistent.index) {
      memoryCache.set(memoryKey, { snapshot, index: persistent.index });
  ...
  if (useCache) {
    ...
    memoryCache.set(memoryKey, { snapshot, index: serialized });
