...
function checkEnvironment(root, add) {
  ...
  const env = process.env.WL_PROJECT_ENV || profile;
  if (env === "prod" && process.env.WL_ALLOW_PRODUCTION_WRITES === "true") {
    add("env-config", false, "生产环境已显式授权 WL_ALLOW_PRODUCTION_WRITES=true", "确认这是受控操作，操作完成立即撤销");
  } else if (env === "prod") {
    add("env-config", true, `profile=${profile}（生产环境，codegen apply 已阻断）`);
  } else {
...
function runDoctor(projectRootInput) {
  ...
  const tenantEvidence = treeContains(root, ["TenantLineInnerInterceptor", "AuthUtil.getLoginCompanyId"]);
  add("tenant", tenantEvidence, "租户拦截器或显式 AuthUtil 证据", "显式配置 tenant.mode 并接入租户实现");
