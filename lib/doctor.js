...
function readBootstrapProfile(root) {
  ...
  return bootstrap && bootstrap.profile
    ? { file: path.join(root, bootstrap.file), profile: bootstrap.profile }
    : null;
}

function detectDbClusterFromDatasource(root, profile) {
  if (!profile) return null;
  const candidates = listConfigFiles(root).filter((file) => new RegExp(`^datasource-${profile}\\.ya?ml$`, "i").test(file.name));
  for (const candidate of candidates) {
    try {
      const content = fs.readFileSync(candidate.abs, "utf8");
      if (/-cx[-.]/.test(candidate.rel) || /url:.*cxdb/i.test(content)) return "cx";
      if (/-non_cx[-.]/.test(candidate.rel) || /url:.*non_cxdb/i.test(content)) return "non_cx";
      if (/-pt[-.]/.test(candidate.rel) || /url:.*ptdb/i.test(content)) return "pt";
    } catch { /* ignore */ }
  }
  return null;
}
