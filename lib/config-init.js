...
const { databaseDefaults, inferDbCluster } = require("./team-database");
...
function buildInitPlan(root, options = {}) {
  ...
  const clusterResolution = inferDbCluster({
    explicit: options.dbCluster,
    project,
    module,
    rootPackage: options.rootPackage,
  });
  if (!clusterResolution.ok) {
    return {
      ok: false,
      projectRoot,
      project,
      module,
      reason: clusterResolution.reason,
      hint: clusterResolution.hint || `dbCluster 只能是 ${clusterResolution.allowed.join("/")}`,
      allowedDbClusters: clusterResolution.allowed,
      actions: [],
    };
  }
  const dbCluster = clusterResolution.cluster;
  const dbDefaults = databaseDefaults(dbCluster);
  ...
  actions.push({
    ...
    content: render(template("bootstrap.yml.tmpl"), { project, datasourceType, dbCluster }),
    action: "write",
  });
  ...
  for (const env of ["dev", "sit", "uat", "pre", "prod"]) {
    actions.push({
      ...
      env,
      rel: `.env.${env}.example`,
      content: renderEnvExample({ project, port, datasourceType, dbCluster, dbDefaults, env, customer, module }),
      action: "write",
    });
  }
  ...
  actions.push({
    ...
    content: renderEnvMatrixSkeleton({ project, module, port, datasourceType, dbCluster, dbDefaults, customer }),
    action: "write",
  });
  ...
  return {
    ...
    datasourceType,
    dbCluster,
    dbClusterSource: clusterResolution.source,
    customer,
    currentProfile,
    overwrite,
    actions,
...
function renderEnvExample(ctx) {
  ...
  const lines = [
    ...
    `DB_CLUSTER=${ctx.dbCluster}`,
    `DB_HOST=db-${ctx.env}`,
    `DB_PORT=${ctx.datasourceType === "oracle" ? 1521 : 3306}`,
    `DB_SID=${ctx.datasourceType === "oracle" ? "ORCL" : ctx.dbDefaults.sid}`,
    `DB_USERNAME=${ctx.datasourceType === "oracle" ? "dbuser" : ctx.dbDefaults.username}`,
...
function renderEnvMatrixSkeleton(ctx) {
  ...
  return [
    ...
    "        prod: prod",
    "    datasource:",
    `      cluster: ${ctx.dbCluster}`,
    `      type: ${ctx.datasourceType}`,
    "      dev:",
    '        host: "db-dev"',
    `        port: ${dbPort}`,
    `        sid: "${ctx.datasourceType === "oracle" ? "ORCL" : ctx.dbDefaults.sid}"`,
    `        username: "${ctx.datasourceType === "oracle" ? "dbuser" : ctx.dbDefaults.username}"`,
    "      prod:",
    '        host: "db-prod"',
    `        port: ${dbPort}`,
    `        sid: "${ctx.datasourceType === "oracle" ? "ORCL" : ctx.dbDefaults.sid}"`,
...
function applyInitPlan(plan, options = {}) {
  if (!plan.ok) return { ok: false, reason: plan.reason, hint: plan.hint, applied: [] };
