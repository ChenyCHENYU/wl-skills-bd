...
3. 更新根 `CHANGELOG.md`
4. 执行 `npm run verify`、`npm run verify:quality-maven` 与 `npm pack --dry-run --ignore-scripts`

## 命名约定（本包内）

- standards 文件：`{编号2位}-{kebab-case}.md`
- SKILL 目录：`skills/{core|data|test|ops|domain}/{kebab-name}/`
- SKILL 文件：`SKILL.md` + `USAGE.md`（必需）
- ADR：递增编号，不复用
...
- `files/CLAUDE.md` / `files/AGENTS.md` 是安装资产源，修改后必须执行安装器与版本一致性测试；当前没有二次文档生成器。
