...
L4 工程产物  Java/XML/DDL/tests/contracts/catalog/docs + standards/skills/quality config
                    │
                    ▼
L5 验证  B1~B25 + J1~J8 + strict contract diff + assurance evidence + package self-check
                    │
                    ▼
...
| `lib/be-rules.js` | B1~B25 只读扫描与证据 | 自动修复 |
...
1 个 Skill 仍是流程骨架：业务文档抽取；行为测试生成已由 `test gen/scenarios` 落地，db-migration 对复杂数据回填仍是部分能力。环境配置已由 config 命令族落地。没有对应执行器的能力不得在 README/USAGE 中宣称自动 apply。
