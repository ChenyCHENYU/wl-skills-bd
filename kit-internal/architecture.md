...
> 状态：accepted · 版本：0.13.0 · 日期：2026-07-18
...
已评审需求 / 可选 design-model 或前端契约 / 数据库约束
                    │
                    ▼
L0 机器事实  JSON Schema + shared delivery profile + rule catalog
                    │
                    ▼
L1 确定性核心  install / contract / codegen / collaboration / audit / safe-fix / config / task-router
                    │
          ┌─────────┴─────────┐
          ▼                   ▼
L2 CLI 适配              L2 MCP 适配
          ...
          └─────────┬─────────┘
                    ▼
L3 工程产物  Java/XML/DDL/tests/contracts + standards/skills/quality config
                    │
                    ▼
L4 验证  B1~B23 + J1~J8 + strict contract diff + package self-check
                    │
                    ▼
L5 人工卡口  DDL/数据、权限发布、环境部署、破坏性 API、业务重构
```
...
| `lib/codegen.js` | 16 产物、planHash、业务保护区、冲突、事务回滚与完成度证据 | 执行 DDL |
...
生成器不伪造 export、relation 或缺少确定性 patch 的业务逻辑。这些方法初始为 draft，占位内容和对应测试位于具名 `<wl-custom>` 区。重生成会按名称保留区域，区外继续严格受管；完成度由“方法已去占位 + ServiceTest 有调用证据”共同确定。
