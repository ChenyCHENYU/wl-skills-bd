...
(async () => {
  ...
  const contractInspect = await HANDLERS.wls_be_contract.handle({ mode: "inspect", contract });
  assert.strictEqual(contractInspect.structuredContent.contractKind, "crud");
  assert.strictEqual(contractInspect.structuredContent.codegenSafe, true);
  const integrationInspect = await HANDLERS.wls_be_contract.handle({ mode: "integration-inspect", contract });
  assert.strictEqual(integrationInspect.structuredContent.readiness, "not-declared");
  ...
  assert.strictEqual(validateSchema(contractSchema, { mode: "inspect", contract }).valid, true);
  assert.strictEqual(validateSchema(contractSchema, { mode: "impact", module: "mdm", field: "featureName", limit: 20, cursor: 0 }).valid, true);
  assert.strictEqual(validateSchema(contractSchema, { mode: "impact", field: "featureName" }).valid, false);
  ...
  assert.strictEqual(validateSchema(validateToolSchema, { rules: ["B32"] }).valid, true);
  assert.strictEqual(validateSchema(validateToolSchema, { rules: ["B33"] }).valid, false);
  ...
  const catalogSchema = TOOLS.find((tool) => tool.name === "wls_be_catalog").inputSchema;
  assert.strictEqual(validateSchema(catalogSchema, { mode: "show", module: "mdm", section: "apis", limit: 20, cursor: 0 }).valid, true);
  assert.strictEqual(validateSchema(catalogSchema, { mode: "integration-audit" }).valid, false);
