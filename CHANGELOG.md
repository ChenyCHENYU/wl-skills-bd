...
- 合并 v0.21.0 的规则短路、Source Index、统一 MCP 输出预算和 Pipeline 能力时保留双方契约，避免覆盖已发布能力。
- Skill 能力目录支持标准 `metadata.status/stage` 嵌套前置元数据，阻断 12 个 Skill 被误判为 unknown。
- DDL 账本记录时间由执行证据确定，保证预览与确认阶段 planHash 稳定；双哈希回执重复提交保持幂等。
...
- 新增 B30：从 Controller 源码提取 HTTP 方法、类级路径、方法级路径、处理方法与行号，输出真实端点清单并阻断重复完整路由。
- `runBeRules` 结构化结果新增 `endpoints[]`，便于与前端调用清单、机器 API 契约和网关路由做后续确定性对账。

### Changed

- 端点盘点不假设查询必须 GET，也不绑定具体服务或业务路径；POST 查询及项目自定义方法按实际注解原样记录。
- 动态常量路径不做语义猜测，仍应由编译后的 Controller 契约测试/OpenAPI 证据补齐，避免静态扫描误报。

## [0.18.1] - 2026-08-05（安装事务安全补强）

### Added

- installer 增加文件/目录/manifest 写入日志；任一复制或落盘异常时自动恢复执行前内容、清理本次失败事务备份，并返回 `write-failed-rolled-back` 结构化原因。
- 安装器回归新增“先更新成功一项、后续源文件失败”的故障注入，验证业务目录和原 manifest 均恢复。

### Fixed

- 修复安装过程中发生 I/O 异常时可能留下部分新文件或新旧内容混合的问题。
- CLI 对安装失败给出明确原因，并在回滚成功时明确说明未留下半安装结果。

## [0.18.0] - 2026-08-04（项目口径与边界契约闭环）

### Added

- 契约新增写入/查询独立约束、约束来源、客户端/服务端上下文和跨字段时间顺序；DTO、manifest、api.md 与前后端握手使用同一机器事实。
- 标准 CRUD 的方法、路径、Controller 注解和请求绑定由项目 Delivery Profile 驱动；内置 POST/1/10/200 仅作无项目配置时的安全基线。
- B26 增加本地 `mapper-locations` 与实际 Mapper XML 资源路径闭环；B29 按生效 Profile 校验 PageDTO 默认值和上限。
- B28 容器测试门要求按扩展点接口唯一解析 Bean，并为装饰器提供必要委托调用证据。

### Fixed

- 数据库列宽不再机械套用到查询 DTO；文本字段不会因命名或共享模型被误套数字规则。
- 项目显式采用 GET、默认每页 20 或其他安全口径时不再被通用基线误报；真正的契约/代码漂移仍阻断。
- 本地 Mapper XML 位于嵌套资源目录且配置 glob 不覆盖时，在启动前给出明确 B26 问题。
...
- 转义顶层帮助文本中的 `${VAR}` 示例，避免 Node 将其当作模板插值并触发 `ReferenceError`。

### Verification

- CLI 回归新增 `help` 进程级测试，验证退出码和占位符原样输出。
