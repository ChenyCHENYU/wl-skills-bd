...
8. API/E2E 前置验证登录 token、company/tenant、权限与环境；写后读必须使用同一租户。缺公司上下文应标为测试前置失败，不得误报“写入成功但查询不可见”。
9. 查询至少覆盖正向命中、无命中、重置和分页上限；业务去重覆盖新增、更新排除自身、他值冲突和软删重建；批量操作覆盖 selected-only 正向、少选、跨组/不一致/状态拒绝，并对每个负向断言零写入。
