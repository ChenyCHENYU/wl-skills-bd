...
```
必读：当前任务路由命中的 standards 子集；全量审计时读 01~29
...
```
必读：03 / 04 / 11 / 13
按需：07（涉及响应结构改造时）/ 08 + 22（涉及跨系统逻辑 ID、投递与重试时）
```
...
> **不要** 一次性读取全部 29 条。错误示范：`✅ 已读取 standards/01 ~ standards/29`。
