...
- 文档镜像中的字段同样受命名治理：发现 `is_` 前缀必须输出 B31 告警；受管项目直接阻断建表/加字段，并建议改为 `*_flag`。不得因为“需求文档就是这样写的”绕过 Java Bean get/set 与序列化兼容风险。
...
- 更不允许手工 ALTER/DML 后再让 Flyway “补录成功”。受管迁移必须由 Flyway 作为唯一写入者；已发生的计划外变更按漂移事件停止处理，并用新的前向协调迁移恢复四方一致。
...
- 禁止新增 `is_` 数据库列或 `isXxx` Java 属性；需求文档存在此类命名时先确认并修订为 `*_flag`/`xxxFlag`。contract 阶段仍允许显式引用并删除旧 `is_` 列，避免规则妨碍存量治理。
...
- 2026-09-19 v0.27：新增 `is_`/`isXxx` 命名禁令，需求镜像告警、契约与 DDL 新增强阻断，默认软删字段收敛为 `DELETE_FLAG/deleteFlag`。
