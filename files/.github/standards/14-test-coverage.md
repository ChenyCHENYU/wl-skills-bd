...
新增 `MetaObjectHandler`、序列化器、异常处理器、MyBatis 插件等框架 SPI Bean 时，普通 `new` 对象单测只能证明方法行为，不能证明 Spring 装配正确。至少增加一个不依赖 Nacos、Redis、共享数据库的最小上下文测试，同时注册平台 Bean 与业务 Bean，并按扩展点接口类型取 Bean，断言：

- 上下文成功刷新，无 `NoUniqueBeanDefinitionException`；
- `context.getBean(扩展点接口.class)` 能唯一解析预期业务装饰器；
- 装饰器要求保留平台语义时，使用 mock/spy 验证每个必要委托方法确实被调用；
- 接口类型只解析到设计指定的首选实现；
- 若采用装饰器，平台 Bean 仍被调用，原有框架副作用不丢失；
- 空值、空字符串和已有合法值等边界均有行为断言。

该测试属于配置类的功能门禁，不因“配置类不统计覆盖率”而省略（B28）。
...
- 跨服务 JSON DTO：按真实 Jackson 命名策略执行序列化→反序列化往返，逐字段断言；至少覆盖一个正常对象和布尔值、数字、可空字段等边界。只有全参构造且没有无参构造/`@JsonCreator` 的模型必须由 B32 阻断。
...
- 2026-08-04 v0.18.0：框架扩展点测试增加按接口唯一解析和委托调用证据；补充 Profile 分页、写入/查询约束与跨字段时间边界的正反例。

- 2026-08-03 v0.17.10：新增框架扩展点 Bean 的最小 Spring 容器装配测试门禁。
