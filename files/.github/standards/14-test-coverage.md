...
- 身份前置：token 有效、company/tenant 非空、权限和环境正确；写后读使用同一租户。身份不完整时测试应 fail-fast，不得把隔离结果上报为数据不可见缺陷。
- 查询：正向命中、无命中、重置、模糊中间值和 Profile 最大分页值。
- 业务去重：新增冲突、修改自身不误判、修改为他值被拒、软删后重建策略；错误消息必须包含业务对象。
- 批量/统合：只校验已选记录、最少选择数、一致字段、恰好分组数和状态门禁；每个负向均断言 revision/状态/关联零变化。
