...
核心流程：已评审需求/可选上游契约 → `wl-contract.json` → codegen plan → 用户评审 planHash → apply → `wl-api-contract` strict diff → B1~B25 → `mvn verify -Pwl-quality`。生产契约还必须满足 standards/28 的证据链；design/kit 都不是 bd 的硬依赖。
