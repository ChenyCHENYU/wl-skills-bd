...
7. 最终验证执行 B1~B30 与 `mvn verify -Pwl-quality`（J1~J5/J8）；J6/J7 不得冒充默认硬门。新增框架 SPI Bean 必须有最小 Spring 容器装配测试，并按接口类型解析唯一 Bean、验证必要委托；Controller 端点清单必须与契约/前端调用对账。
...
wl-skills-bd validate . --strict           # B1~B30（含真实 Controller 端点清单）
