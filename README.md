...
[![Status](https://img.shields.io/badge/status-v0.13.0-blue.svg)]()
[![Node](https://img.shields.io/badge/node-%3E%3D22-green.svg)]()
[![JDK](https://img.shields.io/badge/JDK-8-blue.svg)]()
[![Standards](https://img.shields.io/badge/standards-26-orange.svg)]()
...
| 契约生成 | 严格 `wl-contract.json` → 14 个模板产物 + 2 个前后端协作产物 |
| 业务扩展（v0.9） | `customOperations` 业务命令/状态机、`relations` 主从关联、`alter` ALTER TABLE、`indexes` 自定义索引、可选 `export`、`externalId` 跨包桥接 |
| 数据安全护栏（v0.10） | B13~B19：Redis TTL/Redisson 锁/禁用命令、物理删禁令/全表写禁令/批量分批、生产只读护栏、二次确认 |
| 稳定性与多环境（v0.11） | B20~B23：事务内 MQ·HTTP/Swagger 混用/巨型 Service；定时任务规范、多环境分支模型、生产护栏运行时强制 |
...
| 快速审计 | B1~B23（含 Redis/敏感写/限流熔断/定时任务/Swagger/巨型 Service）；text/JSON/Markdown/SARIF |
...
| AI 接入 | 12 个 MCP 工具，CLI/MCP 复用同一 `lib/` 实现和同一 JSON Schema |
...
契约必须明确：Profile、根包、模块、Entity/表、外部网关路径、五类权限码（可选第六类 export）、Oracle/MySQL、迁移版本、恢复策略、验证 SQL 和字段模型。生成结果共 16 个：
...
- 2 个 DDL 资产：正向 migration（CREATE 或 ALTER）、人工恢复说明（ALTER 含 Expand-Contract 阶段）；
...
| 扩展字段 | 生成内容 | 典型场景 |
|---|---|---|
| `customOperations[]` | Controller 方法 + Service 四段式（校验存在→校验前置→构造 patch→updateById）；batch 返回 successCount/failureCount/failedIds | submit/approve/reject/withdraw/changeStatus/convert/release/close/cancel/batchXxx |
| `relations[]` | 主 Controller 的 queryXxxByParentId 接口；manifest 暴露关联契约 | 订单/明细、配置/子项、主从表 |
| `alter{}` | ALTER 迁移 SQL（add/drop/modify）；Rollback.md 含 Expand-Contract 阶段 | 加字段、改长度、删废弃字段 |
...
| `wls_be_standards` | 否 | 读取 26 条规范 |
| `wls_be_templates` | 否 | 读取 14 个模板 |
...
11 个 Skill 中 8 个已落地，1 个部分落地（db-migration：CREATE/ALTER/索引已自动生成），2 个仍是诚实标记的流程骨架：业务文档抽取、业务级单测生成。环境配置已由 config init/migrate/doctor/fix 落地；骨架能力没有执行器时不展示虚构命令，也不承诺自动应用。
...
**多环境与分支模型**（standards/24）：
- 5 环境矩阵：dev/sit/uat/pre/prod + Nacos namespace 隔离
- 5 级分支：master/pre/uat/slt/dev + `dev-{模块}-{工号}`
- 业务中心 × 端口 × 数据库集群（cx/non_cx/pt）映射固化
- 生产护栏运行时强制：`WL_PROJECT_ENV=prod` 时 codegen/safe-fix apply 零写入，需 `WL_ALLOW_PRODUCTION_WRITES=true` 显式授权
...
生产环境 codegen/safe-fix/permissions apply 默认阻断；敏感操作须二次确认。详见 [ops/data-safety](files/.github/skills/ops/data-safety/SKILL.md)。
...
| 任务 | 模式 | 触发词 | 规则子集 |
|---|---|---|---|
| new-service | full | 新开发/全套CRUD | B1-B23 + J |
| add-api | incremental-contract | 加接口/加方法 | B1/B2/B5/B8/B12/B20 |
| add-field | incremental-contract | 加字段/落库 | B3/B4/B7/B18 |
| add-business-cmd | incremental-contract | 加submit/状态机 | B5/B8/B17/B20 |
| fix-bug | fix | 改bug/修复 | B3/B5/B7/B8/B17/B18 |
| refactor | fix | 重构/优化 | B5-B12/B23 |
| audit | readonly | 审计/体检 | B1-B23 |
