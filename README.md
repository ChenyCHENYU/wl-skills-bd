...
12 个 Skill 中 9 个已落地，1 个部分落地（db-migration：CREATE/ALTER/索引已自动生成），2 个仍是诚实标记的流程骨架：业务文档抽取、业务级单测生成。项目上下文由 catalog/context 执行器落地；环境配置由 config init/migrate/doctor/fix 落地；骨架能力没有执行器时不展示虚构命令，也不承诺自动应用。
