...
| 边界与项目口径（v0.18） | 写入/查询约束分离、跨字段时间顺序、客户端/服务端上下文、Profile 驱动 HTTP 与分页、B26 Mapper 资源路径和 B29 分页 DTO 闭环 |
...
| 快速审计 | B1~B29（含 Redis/敏感写/方法安全/敏感日志/Mapper 绑定与资源位置/父 BOM 依赖/框架扩展点 Bean/Profile 分页/限流熔断/定时任务/Swagger/巨型 Service）；text/JSON/Markdown/SARIF |
